...
const { directory: _publishConfigDirectory, ...publishConfigForPublish } =
  publishConfig ?? {};
const hasPublishDirectory = Boolean(publishConfig?.directory);
...
if (generateTypings) {
  ...
  if (publishTypingEntryOnly) {
    ...
    await emptyDir(typingsDir);
    await outputFile(typingEntryPath, typingEntry, "utf8");
  }
}
...
await Promise.all([
  writeJson(path.join(outputDir, "dist-cjs", "package.json"), {
    type: "commonjs",
  }),
  writeJson(path.join(outputDir, "dist-es", "package.json"), {
    type: "module",
  }),
]);
...
for (const file of FILES_TO_COPY) {
  ...
  if (file === "LICENSE" && !(await pathExists(filePath))) {
    filePath = path.join(repoRoot, file);
  }
  try {
    await fs.cp(filePath, path.join(outputDir, file), { recursive: true });
  } catch (error) {
...
async function outputFile(filePath, content, encoding = "utf8") {
  await fs.mkdir(path.dirname(filePath), { recursive: true });
  await fs.writeFile(filePath, content, encoding);
}

async function writeJson(filePath, value, { spaces } = {}) {
  await outputFile(filePath, JSON.stringify(value, null, spaces));
}
...
async function transformWorkspaceDeps(dependencies = {}) {
  ...
  for (const [name, version] of Object.entries(dependencies)) {
    ...
    const range = version.slice(10);
    if (!["~", "*", "^"].includes(range)) {
      workspaceDependencies[name] = range;
      continue;
    }
    const dependencyManifest = (
      await import(`${name}/package.json`, { with: { type: "json" } })
    ).default;
    workspaceDependencies[name] =
      `${range !== "*" ? range : ""}${dependencyManifest.version}`;
  }
