...
- These match the commit `389640594 chore(mcp): quarantine 7 pre-existing test failures unrelated to the cleanup` — i.e. they're known and explicitly carved out. 14 Jun closed 4 of the 5 in-tree (see Section 7 item #5); only the theme-data failure remains.
...
5. **Fix or formally retire the 5 quarantined tests** before publishing 0.1. **⚠️ PARTIAL 14 Jun (4 of 5 closed).** In-tree fixes:
   - (a) `contextCoverageProduction.spec.ts` snapshot updated to acknowledge the new `SaltProviderNext` registry entity (1 component coverage gap added to the expected catalog; the registry build had added the entity since the test's last snapshot).
   - (b) `publicContractParity.spec.ts > upgrade full semantics` fixed by setting `include_deprecations: false` as the default in MCP's `upgrade_salt_ui` execute handler, matching the CLI's `salt-ds upgrade` default (was silently defaulting to `true` via `compareVersions`, downgrading confidence to `medium` on calls that hadn't asked for deprecation analysis). Fixture `upgrade.full.json` regenerated to capture the now-correct `confidence_level: "high"`.
   - (c) `registry.integration.spec.ts > extracts getting-started guides...` fixed by opting [site/docs/getting-started/developing.mdx](site/docs/getting-started/developing.mdx) into the `salt_ai_guide: true` frontmatter convention (the registry build's getting-started extractor gates on this flag; the file was simply not opted in) and changing the test query from `"bootstrap"` (zero occurrences in the source) to `"developing"` (the basename-derived alias the inferrer already synthesizes). No prose edits to user-facing docs.
   - (d) `registry.integration.spec.ts > uses structured MDX extraction...` fixed by loosening the positive anchor from `"Amplitude font"` (does not appear in any extracted block) to `"Amplitude"` (appears in the prose `"The JPM Brand theme requires the **Amplitude** web font."`). The real test invariants — no raw fenced code blocks, no `src: local(...)` CSS — are preserved unchanged.

   **1 remaining failure is out-of-scope** for the AI-packages branch: 1 theme-data drift (`themeDeprecatedTokenReplacementMetadata.spec.ts` — 54 stale `basis` line ranges in [packages/theme/css/deprecated/token-replacements.json](packages/theme/css/deprecated/token-replacements.json), owned by the theme team).
