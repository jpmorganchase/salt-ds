...
import { buildProjectPolicyReviewGuidanceCandidates } from "@salt-ds/semantic-core/tools/workflowProjectPolicyApplication";
...
export function buildReviewFixCandidates(
  ...
  input: {
    projectPolicy?: WorkflowProjectPolicyArtifact | null;
  ...
  for (const file of sourceValidation.files) {
    ...
    for (const candidate of buildProjectPolicyReviewGuidanceCandidates({
      code: input.codeByPath?.get(file.path) ?? "",
  ...
  const notes =
    totalCount > 0
      ? [
          ...
          ...(input.projectPolicy
            ? [
                "Project-policy fix candidates can still require follow-up even when canonical Salt validation is otherwise clean.",
              ]
            : []),
        ]
      : [];
