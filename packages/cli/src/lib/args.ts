...
const ROOT_HELP = [
  ...
  "  salt-ds review [target ...] [--url <url>] [--create-report <path>] [--migration-report <path>] [--report <path>] [--validate <report.json>] [--resume <report.json>] [--json] [--json-file <path>] [--registry-dir <path>] [--timeout <ms>] [--mode <auto|browser|fetched-html>] [--full] [--hook] [--emit-attestation] [--verify-attestations]",
...
const COMMAND_HELP: Record<string, string[]> = {
  ...
  review: [
    ...
    "",
    "Usage:",
    "  salt-ds review [target ...] [--url <url>] [--create-report <path>] [--migration-report <path>] [--report <path>] [--validate <report.json>] [--resume <report.json>] [--json] [--full] [--hook] [--emit-attestation] [--verify-attestations]",
    "",
    "Purpose:",
    "  Validate existing Salt source against canonical docs, registry evidence, migration reports, and optional runtime evidence.",
    "",
    "Hook mode:",
    "  --hook reads VS Code / Claude Code / Copilot CLI agent hook JSON on stdin (see https://code.visualstudio.com/docs/agent-customization/hooks).",
    ...
    "",
    "Attestation mode:",
    "  --emit-attestation emits a Zod-typed attestation payload (salt-ds.dev/schemas/attestation/v1) on stdout when a PostToolUse review is clean. One NDJSON line per emit. Pipe stdout to the audit store you already operate (git notes, signed commits, check API, SIEM, plain file). Salt does not pick the disk layout, the hashing algorithm, the retention policy, or the GC story.",
