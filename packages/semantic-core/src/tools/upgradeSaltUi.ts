...
export interface UpgradeSaltUiInput {
  package?: string;
...
export interface UpgradeSaltUiResult {
...
export function upgradeSaltUi(
  ...
  const guidanceBoundary = buildGuidanceBoundary({
    workflow: "upgrade_salt_ui",
    has_deprecations: deprecations.length > 0,
  });
  ...
  return {
    ...
    next_step: appendProjectConventionsNextStep(
      summary.next_steps[0] ?? result.next_step,
      guidanceBoundary,
    ),
    docs: summary.docs,
    notes: mergedNotes,
