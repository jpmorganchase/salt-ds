...
const VALIDATION_ISSUE_CATALOG: Record<string, ValidationIssueDescriptor> = {
  ...
  "tokens.unknown-salt-token": {
    category: "tokens",
    rule: "only-use-known-salt-token-names",
    severity: "warning",
    title: "Unrecognized Salt token name detected",
    message:
      "Styling references a --salt-* token name that is not part of the canonical Salt token registry.",
    suggested_fix:
      "Replace the token with a documented Salt token or confirm that the token is repo-local rather than canonical Salt.",
    confidence: 0.88,
    canonical_source: DESIGN_TOKENS_DOC_URL,
    source_urls: [DESIGN_TOKENS_DOC_URL],
  },
  ...
  "primitive-choice.native-table": {
    ...createPrimitiveChoiceDescriptor({
      ...
      message:
        "This data surface recreates a tabular Salt UI with raw HTML table markup instead of using the Salt Table or Data grid path.",
      suggested_fix:
        "Use Salt Table for standard tabular display, or use Data grid when the experience needs richer grid behavior such as large datasets, sorting, or filtering.",
      confidence: 0.84,
      related_components: ["Table", "Data grid"],
      extra_steps: [
        "Replace raw table markup with the nearest Salt table guidance so structure, density, and interaction stay aligned with the design system.",
      ],
    }),
  },
