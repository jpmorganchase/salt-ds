...
import { ARCHETYPE_PRIORITY, UI_ARCHETYPES } from "./sourceUiArchetypes.js";
import type {
  DetectionBundle,
  DetectionSignal,
  RegionSignal,
...
const REGION_PATTERNS: Array<{
  ...
  },
  {
    ...
    query_patterns: [
      /\b(sidebar|side nav|sidenav|navigation rail|left nav)\b/i,
    ],
...
function addDetectionSignal(
  ...
  const current = detections.get(kind) ?? {
    kind,
...
function addRegionSignal(
  ...
  const current = signals.get(kind) ?? {
    kind,
    evidence: new Set<string>(),
    matched_sources: new Set<string>(),
    from_code: false,
...
export function detectFromOutline(
  ...
  for (const actionValue of outline.actions ?? []) {
    ...
    for (const archetype of UI_ARCHETYPES) {
      if (
        !archetype.query_patterns?.some((pattern) => pattern.test(normalized))
      ) {
        continue;
      }
  ...
  for (const note of outline.notes ?? []) {
    ...
    merged.detections.forEach((value, key) => {
      detections.set(key, value);
    });
    merged.region_signals.forEach((value, key) => {
      regionSignals.set(key, value);
    });
    merged.state_signals.forEach((value, key) => {
      stateSignals.set(key, value);
    });
  }
  ...
  for (const source of sources) {
    for (const [kind, signal] of source.entries()) {
      ...
      if (current) {
        ...
        for (const value of signal.matched_sources) {
...
export function mergeDetectionBundle(
  ...
  const signals = [...detections.values()].sort((left, right) => {
    const priorityDelta =
      ARCHETYPE_PRIORITY.indexOf(left.kind) -
      ARCHETYPE_PRIORITY.indexOf(right.kind);
    if (priorityDelta !== 0) {
  ...
  for (const signal of signals) {
    sortedDetections.set(signal.kind, signal);
  }
