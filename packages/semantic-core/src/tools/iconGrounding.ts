import { getIconLiteRecords } from "../registry/runtimeCache.js";
...
function getAvailableIconRecords(registry: SaltRegistry): IconLiteRecord[] {
  const registered = getIconLiteRecords(registry);
  ...
  for (const imported of saltIconImports) {
    ...
    issues.push({
      ...
      evidence: [
        ...
        "The public MCP v1 package grounds icon usage through icons-lite.json rather than the full icon registry.",
        ...(suggestion
