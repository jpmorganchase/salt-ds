...
function getDiscoverSaltConventionsHint(
  ...
  if (options.routed_workflow === "migrate_to_salt") {
    return {
      supported: true,
      contract: "project_conventions_v1",
      check_recommended: true,
  ...
  if (options.routed_workflow === "create_salt_ui") {
    return getCreateSaltUiConventionsHint(
...
function getUpgradeSaltUiConventionsHint(
  options: GuidanceBoundaryOptions,
): ProjectConventionsHint {
  const topics = uniqueTopics([
      ...
      : options.workflow === "migrate_to_salt"
        ...
        : options.workflow === "get_salt_entity"
          ...
          : options.workflow === "get_salt_examples"
            ...
            : options.workflow === "review_salt_ui"
              ? getReviewSaltUiConventionsHint(options)
              : options.workflow === "upgrade_salt_ui"
                ? getUpgradeSaltUiConventionsHint(options)
