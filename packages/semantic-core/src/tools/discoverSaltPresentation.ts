  ...
  return (result.recommendations ?? []).map((recommendation) => {
    ...
    return {
      ...
      docs: relatedDocs
        ...
        : [],
      guidance_sources: toStringArray(recommendationRecord?.guidance_sources),
...
export function toCompactPatternOptions(
  ...
  return (result.recipes ?? []).map((recipe) => {
    ...
    return {
      ...
      guidance_sources: toStringArray(recipeRecord?.guidance_sources),
...
export function getDiscoverySuggestedFollowUps(
  ...
  const projectConventionSuffix = guidanceBoundary?.project_conventions
    .check_recommended
    ? (() => {
        const topics = describeProjectConventionsChecks(
          guidanceBoundary.project_conventions.topics,
        );
        return topics
          ? ` Then confirm ${topics} through project conventions before landing the final project answer.`
          : " Then confirm any relevant project conventions before landing the final project answer.";
      })()
    : "";

  if (decision.workflow === "create_salt_ui") {
    return [
      {
        workflow: "get_salt_examples",
        reason: `Review the best implementation examples after choosing a Salt direction.${projectConventionSuffix}`,
        args: {
          query,
        },
      },
  ...
  if (decision.workflow === "migrate_to_salt") {
    return [
      {
        workflow: "migrate_to_salt",
        reason: `Translate the source UI into Salt targets, grouped regions, and scaffold-ready workstreams before narrowing to final component or pattern details.${projectConventionSuffix}`,
        args: {
          query,
    ...
    return [
      {
        ...
        reason: `Narrow the query after checking the closest docs result.${projectConventionSuffix}`,
        args: {
          query,
        },
      },
...
export function resolveDiscoverNextStep(input: {
  ...
  if (decision?.workflow === "upgrade_salt_ui") {
