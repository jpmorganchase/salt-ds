...
export function buildUsageSemantics(input: {
  category?: string[];
  preferred_for?: string[];
  not_for?: string[];
  derived_from?: UsageSemanticsSource[];
}): UsageSemanticsRecord | undefined {
  const semantics: UsageSemanticsRecord = {
    category: uniqueStrings(input.category ?? []),
    preferred_for: uniqueStrings(input.preferred_for ?? []),
    not_for: uniqueStrings(input.not_for ?? []),
    derived_from: uniqueStrings(input.derived_from ?? []),
  };

  return semantics.category.length > 0 ||
    semantics.preferred_for.length > 0 ||
    semantics.not_for.length > 0
    ? semantics
    : undefined;
}
