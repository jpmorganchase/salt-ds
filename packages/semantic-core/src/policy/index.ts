...
export type MergedProjectConventionsResult = {
  ...
  project_conventions: {
    ...
    theme_defaults: ThemeDefaultsConvention | null;
    token_aliases: TokenAliasConvention[];
    token_family_policies: TokenFamilyPolicyConvention[];
    applied: boolean;
...
function hasConventionData(conventions: ProjectConventions | null): boolean {
  ...
  return Boolean(
    conventions.contract ||
      ...
      (conventions.token_aliases?.length ?? 0) > 0 ||
      Boolean(conventions.theme_defaults) ||
      (conventions.token_family_policies?.length ?? 0) > 0 ||
...
export function composeProjectConventionLayers(
  ...
  const tokenAliases = new Map<string, TokenAliasConvention>();
  const tokenFamilyPolicies = new Map<string, TokenFamilyPolicyConvention>();
  ...
  let themeDefaults: ThemeDefaultsConvention | undefined;
  ...
  for (const layer of layers) {
    ...
    for (const entry of conventions.token_aliases ?? []) {
      tokenAliases.set(entry.salt_name, entry);
    }

    if (conventions.theme_defaults) {
      themeDefaults = conventions.theme_defaults;
    }

    for (const entry of conventions.token_family_policies ?? []) {
      tokenFamilyPolicies.set(entry.family, entry);
    }
  ...
  const composedConventions: ProjectConventions = {
    ...
    ...(approvedWrappers.size > 0
      ...
      : {}),
    ...(tokenAliases.size > 0
      ? { token_aliases: [...tokenAliases.values()] }
      : {}),
    ...(themeDefaults ? { theme_defaults: themeDefaults } : {}),
    ...(tokenFamilyPolicies.size > 0
      ? { token_family_policies: [...tokenFamilyPolicies.values()] }
      : {}),
...
export function mergeCanonicalAndProjectConventionLayers(
  ...
  return {
    ...
    project_conventions: {
      ...
      theme_defaults: composed.conventions?.theme_defaults ?? null,
      token_aliases: composed.conventions?.token_aliases ?? [],
      token_family_policies: composed.conventions?.token_family_policies ?? [],
