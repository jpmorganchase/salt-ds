...
const SALT_REPO_INSTRUCTIONS_LINES = [
  ...
  "- use the compact Salt contract first: `status`, `safety`, `action`, and `summary`; inspect full workflow fields only when deeper artifacts are required",
  "",
...
export const VSCODE_COPILOT_INSTRUCTIONS_TEMPLATE = [
  ...
  "- When Salt returns compact workflow output, read `summary`, `status`, `safety`, and `action` first.",
...
export const VSCODE_SALT_UI_AGENT_TEMPLATE = [
  ...
  "2. When Salt returns compact workflow output, read `summary`, `status`, `safety`, and `action` first.",
  ...
  "4. If compact create output is `blocked`, `partial`, or `safety.exact_request_safe: false`, follow the returned `action` before implementing the blocked region.",
