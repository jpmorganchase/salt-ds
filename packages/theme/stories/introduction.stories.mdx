...
In order to support uses of Salt that utilize external windows. We have created a custom hook that injects component CSS into a given window
...
- All Salt component CSS must be imported using the default export, the imported CSS should be injected by calling `useComponentCssInjection`
- Only one component should be stored per typescript jsx file, however a component may import and inject multiple css files.

In the above example, the call to `useComponentCssInjection` passing in an ID which is used to keep track of if component CSS has already been injected before,
