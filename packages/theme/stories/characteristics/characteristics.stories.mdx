...
Note the use of `-high` emphasis characteristic tokens provided by UITK that weren't used in Card by default, as well as raw values to further emphasise the component. Now, all we have to do to make Card have emphasis is pass in the `uitkEmphasisHigh` prop:
