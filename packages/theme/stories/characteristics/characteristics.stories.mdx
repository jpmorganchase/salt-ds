...
An example of a characteristic token which has emphasis levels is `--uitk-editable-background-` which has the three levels defined, `low`, `medium`, and `high`. By default, the FormField component has been styled to use the `--uitk-editable-primary-background` token in a local variable:

```css
.uitkFormField {
  --formField-background: var(--uitk-editable-primary-background);
}
```
...
.uitkEmphasisHigh.uitkFormField {
  --formField-background: var(--uitk-editable-secondary-background);
}
```
...
In the following example, we will show how to add emphasis to the Card component. Currently, Card makes use of the `---uitk-container-primary-background` token for all of it's styling purposes. Here's how it looks by default:
