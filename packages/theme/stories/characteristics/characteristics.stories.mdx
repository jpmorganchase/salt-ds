...
- Simplified the design of new components and patterns e.g. drag and drop behavior
...
Note how we make use of the `-high` emphasis characteristic tokens provided by UITK that weren't used in Card by default as well as raw values to further emphasize the component. Now, all we have to do to make Card have emphasis is pass in the `uitkEmphasisHigh` prop:
