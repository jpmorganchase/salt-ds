...
### Simple customisation using characteristics

In the following example, we will show how to customise the Card component through characteristics. Currently, Card only has one variant, using tokens such as `---uitk-container-primary-background` all of it's styling purposes. Here's how it looks by default, shown on a purple Panel for demo purposes:
...
Note how no classname is needed to be passed in, we have just redefined the Card's styling at a higher specificity, to produce the following styling:
