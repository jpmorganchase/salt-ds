...
4. Append the attribute, e.g. `--uitk-actionable-background-cta`, `--uitk-text-h2-font-size`, `--uitk-focused-color`
5. Append the state if applicable, e.g. `--uitk-actionable-background-cta-hover`, `--uitk-focused-color-warning`
6. For tokens that are scoped by density\* the suffix should be the suitable density for the defined value, e.g. `--uitk-text-h2-font-size-high`

\* Here, it is best practice to avoid directly using the density invariant values, and skip this step in actual usage - i.e., discard the `-<density>` suffix to get the appropriate value in practice e.g. `--uitk-text-h2-font-size`.
...
```css
.uitkButton {
  background: var(--uitk-actionable-background-primary);
...
theme, --uitk-actionable-background-primary resolves to `var(--uitk-color-grey-60)`, and in dark theme resolves to `var(--uitk-color-grey-300)`, of which foundation tokens then in turn resolve to their respective RGBA values.
...
It is important to note that just because you have assigned a component to a characteristic, does not mean it consumes _all_ of those characteristics. We can freely pick which characteristics we want. A good example of this may be to notice that the `focused` characteristic offers both a border and outline style. From this you would probably want to use an outline if you're setting the outline attribute, and of course border for the border attribute. Yet, there is nothing stopping you from writing `border-style: var(--uitk-focused-outline-style)` instead!
