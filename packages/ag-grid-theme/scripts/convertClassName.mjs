...
glob
    .sync("packages/ag-grid-theme/css/parts/*.css")
    .forEach(async (file) => {
        ...
        walk(ast, (node, item, list) => {
            if (node.type === 'Selector') {
                if (node.children.some((child) => child.type === 'AttributeSelector' && child.value.value === attributeToMatch)) {
        ...
        walk(ast, (node, item, list) => {
            if (node.type === 'AttributeSelector' && node.value.value === attributeToMatch) {
