...
Use a controlled Multiline Input to enforce a character count on the user input. Add the character count as a span adornment to present the remaining characters out of the given limit.

<Canvas>
  <Story id="lab-multiline-input--character-count" />
</Canvas>

- Set the color on the character count span to "--salt-text-secondary-foreground" and font weight to "--salt-text-fontWeight-small" for Salt styling.
