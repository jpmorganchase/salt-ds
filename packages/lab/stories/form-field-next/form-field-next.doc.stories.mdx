...
<Canvas>
  <Story id="lab-form-field-next--helper-text" />
</Canvas>
...
- The prop should be applied to the field, not the control(s) it contains, for expected results.
...
Any custom `id` prop passed to the Form Field will be used within context for the Form Field Label as `label-${formId}` and the Form Field Helper Text label as `helperText-${formId}`.
