...
child window. Those styles can be optimized by querying the children of the window
via ReactDOMServer to figure out which styles to add.

This method of optimizing will not work for child components that are rendered later such
...
Without any optimizing, this method works because all the component's CSS is injected into the
