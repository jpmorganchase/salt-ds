...
import { Pill, SelectablePill } from "@salt-ds/lab";
import {
  CharacteristicUsage,
  CSSClassTable,
...
[Checkbox](/?path=/story/lab-checkbox--feature-input),
[Icon](/?path=/story/core-icon--salt-icon),
...
- Never apply RAG (status) colors to the Pill component to differentiate between
  categories—use [Icon Descriptor](/?path=/story/documentation-icons-icon--page) instead.
- If a Pill enables an action that is fixed, and not subject to change, use a
  [Button](?path=/story/documentation-core-button--page) instead.

<Canvas>
  <Story of={PillStories.FeaturePill} />
</Canvas>
...
<Canvas>
  <Story of={PillStories.Selectable} />
</Canvas>
...
<Canvas>
  <Story of={PillStories.Controlled} />
</Canvas>
