...
const listItems = SimpleListExample.map((item, index) => <ListItemNext
  key={index}
  disabled={[1, 5].includes(index)}>{item}</ListItemNext>);
...
export const ListItemNextExample: Story<ListNextProps> = (props) => {
  return (
    ...
    >
      ...
      <ListItemNext showCheckbox selected>with checkbox selected</ListItemNext>
      <ListItemNext showCheckbox>with checkbox</ListItemNext>
      <ListItemNext itemTextHighlightPattern="bama">with highlight</ListItemNext>
      <ListItemNext selected disabled>selected disabled</ListItemNext>
      <ListItemNext disabled>disabled</ListItemNext>
      <ListItemNext showCheckbox selected disabled>with checkbox disabled selected</ListItemNext>
      <ListItemNext showCheckbox disabled>with checkbox disabled</ListItemNext>
    </ListNext>
  );
};
