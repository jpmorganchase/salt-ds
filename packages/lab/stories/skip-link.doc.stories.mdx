...
- In most cases, the Skip Link should be the _first focusable item_ on the page. It's acceptable to position it after cookie warnings and disclaimers, but it must be placed before the header content in order to fulfil its purpose: enabling users to skip the header content.
