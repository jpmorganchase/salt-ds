...
<Meta title="Documentation/Lab/Number Input" />

# Number Input

The Number Input displays a default numeric value that users can increase or decrease using the controls or by keyboard actions. Users can also manually enter a specific value.

**Usages**

Use Number Input when you need to collect a numeric value from the user.
...
The out-of-the-box Number Input has a default value of 0, an increment value of 1 and a block value of 10, and you can only use it to accept numeric characters.
...
<Canvas of={NumberInputStories.CustomValues} />

## Decimal

By default, Number Input accepts zero decimal places. If the user needs to enter a value with a decimal such as a monetary amount, you can customize the number of decimal places the input accepts.
...
<Canvas of={NumberInputStories.Decimals} />

## Live default value

You can use the Number Input to pull the live value from a source that’s constantly changing—for example, when dealing with live stock prices.
...
<Canvas of={NumberInputStories.LiveDefaultValue} />
...
<Canvas of={NumberInputStories.RefreshButton} />
...
<Canvas of={NumberInputStories.NumericLimits} />
