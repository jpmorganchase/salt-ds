...
export const TooltipNext = forwardRef<HTMLDivElement, TooltipProps>(
  function Tooltip(
    {
    ...
    const onMouseEnter = (e) => contentRef.onmouseenter?.(e);
    const onMouseLeave = (e) => contentRef.onmouseleave?.(e);
    const onFocus = (e) => contentRef.onfocus?.(e);
    const onBlur = (e) => contentRef.onblur?.(e);
    ...
    useEffect(() => {
      if (contentRef) {
        contentRef.addEventListener("mouseenter", handleMouseEnter);
        contentRef.addEventListener("mouseleave", handleMouseLeave);
        contentRef.addEventListener("focus", handleFocus);
        contentRef.addEventListener("blur", handleBlur);
      }

      return () => {
        if (contentRef) {
          contentRef.removeEventListener("mouseenter", handleMouseEnter);
          contentRef.removeEventListener("mouseleave", handleMouseLeave);
          contentRef.removeEventListener("focus", handleFocus);
          contentRef.removeEventListener("blur", handleBlur);
        }
      };
    }, []);
