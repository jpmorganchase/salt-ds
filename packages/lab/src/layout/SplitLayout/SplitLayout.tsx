...
const SplitItem = forwardRef<HTMLDivElement, SplitItemProps>(
  function SplitItem({align, gap, separators, children, side, ...rest}, ref) {
    return (
      <FlexLayout
        align={align}
        gap={gap}
...
export const SplitLayout = forwardRef<HTMLDivElement, SplitLayoutProps>(
  function SplitLayout(
    ...
    return (
      ...
      >
        <SplitItem side="left" {...sideProps}>{leftSplitItem}</SplitItem>
        <SplitItem side="right" {...sideProps}>{rightSplitItem}</SplitItem>
      </FlexLayout>
    );
  }
);
