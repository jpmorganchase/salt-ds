import { ReactElement, forwardRef, useState, MouseEvent, useEffect, useRef, useImperativeHandle, ComponentPropsWithoutRef } from 'react';
import { clsx } from "clsx";
import { useWindow } from '@salt-ds/window';
import { useComponentCssInjection } from '@salt-ds/styles';
import { FlexLayout, makePrefixer, FlexLayoutProps } from "@salt-ds/core";
...
export const Rating = forwardRef<HTMLDivElement, RatingProps>(
  function Rating(
    ...
    const [label, setLabel] = useState(value ? getLabel(value) : "No rating selected");
    
    const handleKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
      if (readOnly) { // Prevent focus movement in read-only mode
    ...
    const handleMouseHover = (_event: MouseEvent<HTMLButtonElement>, value: number) => {
    ...
    const handleItemClick = (_event: MouseEvent<HTMLButtonElement>, value: number) => {
      if (selected === value) {
        if (allowClear) { // Clear the rating if `allowClear` is true
          setSelectedItem(0);
          setReset(true);
    ...
    return (
      <FlexLayout gap={1} align="center" direction={labelPosition === "left" || labelPosition === "right" ? "row" : "column"}>
        {(showLabel || semanticLabels) && (labelPosition === "top" || labelPosition === "left") && (
          <div className={clsx(withBaseName("label"), withBaseName(`label-${labelPosition}`))}>{label}</div>
        )}
        <FlexLayout
          role="radiogroup"
          gap={0.5}
          ...
          {Array.from({ length: max }, (_, index) => (
            <RatingItem
              ...
              isCurrentStarHovered={
                currentHoveredStarIndex > 0 && index + 1 <= currentHoveredStarIndex
              }
        ...
        {(showLabel || semanticLabels) && (labelPosition === "bottom" || labelPosition === "right") && (
          <div className={clsx(withBaseName("label"), withBaseName(`label-${labelPosition}`))}>{label}</div>
        )}
      </FlexLayout>
    );
