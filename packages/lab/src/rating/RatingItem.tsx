import { ReactElement, forwardRef, MouseEvent, useState, ComponentPropsWithoutRef } from 'react';
import { clsx } from "clsx";
import { makePrefixer } from '@salt-ds/core';
import { useWindow } from '@salt-ds/window';
import { useComponentCssInjection } from '@salt-ds/styles';
import { FavoriteIcon, FavoriteSolidIcon, FavoriteOutlineIcon } from "@salt-ds/icons";
...
export interface RatingItemProps extends ComponentPropsWithoutRef<"button"> {
  ...
  */
  onFeedbackItemHover: (event: MouseEvent<HTMLButtonElement>, itemValue: number) => void;
  /**
  *  callback function when feedback item is clicked.
  */
  onFeedbackItemClick: (event: MouseEvent<HTMLButtonElement>, itemValue: number) => void;
  /**
  ...
  function RatingItem(
    ...
    const renderIcon = () => {
      const iconClass = clsx(withBaseName("icon"), {
        ...
        [withBaseName("icon-empty")]: !isActiveState && !isCurrentStarHovered && !isSelectedStyle,
      });
      ...
      const iconToRender = isActiveState ? outlinedIcon : (isCurrentStarHovered || isSelectedStyle ? filledIcon : emptyIcon );
    ...
    return (
      <button
        ...
        role='radio'
        tabIndex={disabled ? -1 : ((readOnly && itemValue === currentRating) || itemValue === currentRating || (itemValue === 1 && currentRating === 0) ? 0 : -1)}
