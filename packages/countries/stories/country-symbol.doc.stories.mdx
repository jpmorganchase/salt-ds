...
We've made the symbols circular as the traditional flag shape can't easily be interchanged easily with other elements, such as icons or avatars. This also standardizes the shape of each flag, even when they have different proportions.
...
When you need several symbols, in a Country Picker for example, it's easier to render a list of all the symbols and their related information by using:
...
The Lazy Country Symbol uses React.lazy to wrap around the component and load a symbol, based on its country code. This allows you to wrap the component or any of its parents into a `Suspense` boundary to specify the loading fallback.

<Canvas>
  <Story id="country-symbols-lazy-country-symbol--lazy-country-symbol" />
</Canvas>
...
To hide the label, pass `aria-hidden=”true”`.
