...
The Lazy Country Symbol component is a wrapper around the Country Symbol component that loads a Country Symbol based on its country code.
This component uses [React.lazy](https://react.dev/reference/react/lazy) to load the country symbol.
This means you can wrap it or any of its parents into a `<Suspense>` boundary to specify the loading fallback.
