...
To display a cell in one of expected validation states, use the `getValidationState` prop on the `GridColumn` omponent. This prop is a function that provides row and column data, and expects as return value one of known validation states, "error", "warning", and "success".
...
Another optional prop to cusomize the validation is `validationType`. This can be either "strong", or "light". The default value is "light". Both options only apply the visual styling. You will need to provide the validation logic yourself.
