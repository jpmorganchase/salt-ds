...
The grid currently allows users to pass interactive elements inside the custom cells, and that interactive element will automatically receive focus when the cell is focused. Avoid passing multiple interactive elements for now since those cells will not have proper keyboard accessibility. If you need multiple buttons per row, render each button in its own column.

If you add an interactive element with complex keyboard functionality, you may have to prevent the keyboard events from bubbling up since some grid keyboard event listeners might clash with the interactive child element and potentially break keyboard interactions.
