...
async function buildSealedRegistryInCleanNode(
  ...
  const generatorUrl = pathToFileURL(
    path.join(REPO_ROOT, "packages/knowledge/scripts/buildKnowledge.mjs"),
  ).href;
  await fs.writeFile(
    launcherPath,
    [
...
beforeAll(async () => {
  ...
  const workspaceRegistryDir = path.join(REPO_ROOT, "packages/knowledge/generated");
  ...
  if (releasePackedRegistryDir) {
    ...
    await expectCatalogRootsByteEqual({
      ...
      dist: path.join(REPO_ROOT, "packages/knowledge/generated"),
