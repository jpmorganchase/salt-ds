...
- Before choosing contour helpers or radii, follow the canonical design page's **Geometry** and **Calibrate the amount of welding** sections. Classify the finished boundary, including unenclosed marks, then calibrate the visible transition at native size. Existing helper defaults and neighboring recipes are starting points, not approval of weld strength.
...
- Preserve the distinction between construction units, normalized reference geometry, fitted exports and rendered pixels. Follow the canonical contribution page's automatic viewBox fitting workflow; do not hand-pad recipes or force both axes to fill. The generator fits variants independently by default, but the contribution page's final retained-landmark criteria take precedence and may require a reviewed shared frame. Keep exact aliases identical and retain the documented brand and full-canvas surface exceptions. Review apparent size after fitting; use the contribution page's documented optical-target workflow for justified whole-icon adjustments, since recipe-wide scaling or translation is canceled by ordinary fitting.
...
- Compare shared marks in positive and inverse form; derive their gesture from common geometry where possible, then judge local placement and counter weight in the final exports. Follow the guide's order for crowded junctions: reduce local softening or simplify detail before changing line weight, while preserving the identifying silhouette.
...
For modifications, follow the contribution page's baseline, finding classification and whole-family acceptance workflow before retaining a correction. Follow the canonical design page's recognition-first review: record the observed object or action before revealing the name, then check its intended meaning and context. For an entire-set size review, use the contribution page's full density/size matrix and report actual coverage. Rebuild misleading silhouettes before refining small details. Apply the contribution page's paired-overlay review to retained features; a clean recipe or an independent fit does not establish stable landmarks in the final pair.

Use the site's Icons catalogue for synonym searches and variant filters, the existing Storybook Icons/Icon and Icon QA stories for catalogue, size and CSS-mask review, and existing component examples for legibility in context. Follow the contribution page for density, background, size, stroke and temporary view-box inspection. Reuse these surfaces for artwork review instead of adding permanent QA stories. Storybook name search does not test website synonyms. Confirm the rendered dimensions.

Inspect native 16px and 12px renders on light and dark backgrounds, plus an enlarged view. Compare generated components at their default and selected override widths with the numeric SVG reference, following the canonical contribution page's stroke and clearance guidance. Review both requested variants and affected siblings in components and CSS masks, which retain baked widths. Check meaningful features, transparent counters, final fitted gaps, proportional line weights, default or documented optical framing and painted bounds. The validator separately checks raw fitted output and reverses the fit for reference-geometry regressions; passing those checks alone does not establish visual quality. For component legibility corrections, follow the contribution page's before/after review in a real component context.
