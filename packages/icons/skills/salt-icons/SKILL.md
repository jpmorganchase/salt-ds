...
- Preserve the distinction between construction units, normalized reference geometry, fitted exports and rendered pixels. Follow the canonical contribution page's automatic viewBox fitting workflow; do not hand-pad recipes or force both axes to fill. The generator fits each outline and solid export independently while keeping exact aliases identical and retaining the documented brand and full-canvas surface exceptions. Review apparent size after fitting; use the contribution page's documented optical-target workflow for justified whole-icon adjustments, since recipe-wide scaling or translation is canceled by ordinary fitting.
