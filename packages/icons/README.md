...
Validation checks inventory, rendering, fitted occupancy and centering, painted bounds, line weights and exact aliases. It uses the fit manifest to reverse export framing for existing recipe-geometry regressions and checks the final fitted exports separately. Review changed artwork in the site catalogue and Storybook at 12px and 16px on light and dark backgrounds, plus an enlarged view. The **Component Legibility** and **All Icon View Boxes** (`AllIconViewBoxes`) stories use the selected density's theme stroke by default and offer an optional stroke override. All Icon View Boxes also supports the actual density-based icon size or fixed 12px/16px/32px/64px sizes, density selection, light/dark backgrounds and name filtering. Check final gaps at the configured width; automated checks do not establish meaning or visual quality.
