// WARNING: This file was generated by a script. Do not modify it manually

import { forwardRef } from "react";

import { Icon, type IconProps } from "../icon";

export type ProgressCancelledIconProps = IconProps;

export const ProgressCancelledIcon = forwardRef<
  SVGSVGElement,
  ProgressCancelledIconProps
>(function ProgressCancelledIcon(props: ProgressCancelledIconProps, ref) {
  return (
    <Icon
      data-testid="ProgressCancelledIcon"
      aria-label="progress cancelled"
      viewBox="0 0 16 16"
      ref={ref}
      {...props}
    >
      <g strokeWidth="var(--icon-strokeWidth, 1)">
        <path
          fillRule="evenodd"
          d="M.25 8a7.75 7.75 0 1 0 15.5 0A7.75 7.75 0 1 0 .25 8M11 3.763 12.237 5 9.591 7.646a.5.5 0 0 0 0 .708L12.237 11 11 12.237 8.354 9.591a.5.5 0 0 0-.708 0L5 12.237 3.763 11l2.646-2.646a.5.5 0 0 0 0-.708L3.763 5 5 3.763l2.646 2.646a.5.5 0 0 0 .708 0Z"
        />
      </g>
    </Icon>
  );
});
