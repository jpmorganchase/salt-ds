...
Generated React icons map these reference widths to the configured primary width **W** while retaining each stroke's ratio to 0.67. The theme defaults to **W = 1.333333 (approximately 4/3) in high and medium densities**, **W = 1.142857 (approximately 8/7) in low density**, and **W = 1 in touch and mobile densities**; the component's fallback is **1** when no stroke variable is defined. At a rendered size of `P` pixels, the primary stroke occupies `W × P / 16` pixels. At size multiplier 1, the 12px high/medium, 14px low and 16px touch/mobile icons therefore each have an **approximately 1 CSS-pixel** primary stroke; an explicit `W = 1` at 12px gives 0.75px, and the 0.67 reference gives 0.5025px. The `schedule-time` hands retain their lighter 0.72 proportion. Resizing scales the artwork and its strokes together.
...
For an individual decision, run `yarn workspace @salt-ds/icons audit:corners --template battery.svg`. Copy the generated entry from `dist/icon-corner-consistency/review-template.json` into `scripts/artwork/corner-reviews.json`, then fill in the actual review. The template starts unresolved. Review the complete unmarked drawing at 12px, 14px and 16px, with all three themed weights, reference 0.67 and maximum fit width 1.5, on light/dark backgrounds in generated components and CSS masks. The generated corner evidence samples 0.67, 1, 4/3 and 1.5; also inspect the 14px low-density render at 1.142857. Account for every inner or uncertain observation. A `retained-sharp` decision requires before/after comparison files at both 12px and 16px; a detector disagreement needs detail evidence. Evidence paths are repository-relative and must exist.
...
The framing transform moves and scales the container's paths, counters and cutouts together while preserving the **0.67-unit primary reference stroke** and every secondary ratio. Shared final-canvas marks are composed after this transform and retain their authored contours. Generated React components use the density's themed primary width: **1.333333 for high/medium**, **1.142857 for low**, and **1 for touch/mobile**, with a fallback of **1** when neither stroke variable is defined. Thinner strokes occupy slightly less of the frame than the 1.5-width fitting target; inspect acute joins at the actual interface width.
...
Use **W = 1.333333** for the high/medium density theme default, **W = 1.142857** for low density, **W = 1** for the touch/mobile default or an explicit 1-unit comparison, and the selected value for any override. Use **W = 0.67** for the numeric SVG reference. If both neighboring edges carry strokes, include both half-widths.

Choose a useful local gap by comparing the related family and its native rendering. Before fitting, a chosen gap `g_min` between parallel strokes requires `D ≥ g_min + (r1 + r2) × W / 2`. The devices phone header has a reference centerline separation of 2 units; its two primary strokes leave a reference-space gap of `2 − W`: **approximately 0.667 units at the high/medium default W = 1.333333**, approximately 0.857 units at the low default W = 1.142857, 0.5 units at the fitting width W = 1.5, 1 unit at W = 1, or 1.33 units at the 0.67 reference width. These values precede export fitting; calculate the final gap from the fitted distance.
...
SendSolid uses the outline’s complete painted contour at the midpoint of the lightest and heaviest themed widths (1 and 4/3), with its established tapered seam subtracted afterward. Its shared frame preserves the plane’s proportions and the final seam coordinates. A fixed filled contour has small optical edge differences as the outline width changes; keep those bounded across all density defaults instead of introducing stroke-cap steps at the seam mouth.
...
Add a focused final-render regression for a repaired edge: sample the complete SVG, including overlaid strokes, at reference **0.67**, theme **1**, **1.142857** and **1.333333**, and maximum fit width **1.5**. The cutout contour checks cover the bell and cloud-sync openings, music beam, and headphone joins without reversing the export fit. The final cutout-spacing checks compare side, terminal, corner and curve-end gaps at reference 0.67, theme endpoints 1 and 4/3, and maximum width 1.5; they complement the contour checks, which catch steps and overpaint. Keep expected spacing independent of the cutout path being tested. Verify the regression fails on the previous drawing, then inspect native and enlarged renders.
...
Inspect each changed variant and its affected family at native **12px, 14px and 16px** on **light and dark** backgrounds, plus an enlarged view. Check generated components at the themed defaults **1.333333 in high/medium**, **1.142857 in low**, and **1 in touch/mobile**, compare the numeric SVG reference at **0.67**, and inspect the configured weights used by the interface. Temporarily set `--salt-size-icon-strokeWidth` on the review container to compare explicit widths through **1.5**; verify final fitted gaps and bounds at each chosen width.
Use existing size controls where available. For an exact native size, temporarily set `--saltIcon-size` to `12px`, `14px` or `16px` and confirm its computed dimensions; the `size` prop is a density-based multiplier. Use `32px` or `64px` for enlarged inspection and a temporary CSS outline on the SVG to inspect its view box. Keep these review adjustments in the browser rather than adding permanent QA stories.
...
2. Inspect the numbered locations and the complete unmarked drawing, including all exposed sectors. Confirm native 12px/14px/16px recognition, both backgrounds and all density defaults, including the low-density width 1.142857 alongside the map's four sampled widths. The detector is a conservative geometry aid and can miss meaningful relationships; do not restrict visual review to its dots.
...
- [ ] Native 12px, 14px and 16px renders work on light and dark backgrounds; enlarged views reveal no malformed geometry or unintended pinholes inside welded joins, including at the thin reference width.
...
Passing alignment does not approve weld strength, curvature, clear arms, visible weight or small-size recognition. Inspect the native 12px/14px/16px before/after family, actual components and CSS masks as usual. A tangent approach should look natural rather than introducing an abrupt flat or bump into the curve.
...
### Reuse standing figures and entrances

`standing-person.mjs` owns the shared head, shoulder, garment and leg construction used by Man, Woman and Man–woman. The combined icon translates those figures without resizing them. Its shared optical frame preserves head size and foot height; the wider dress intentionally offsets the painted bounding-box center slightly from the head grid. `check-standing-family.mjs` compares the final exported figures after translation at widths 0.67, 1, 4/3 and 1.5.

`open-entrance.mjs` keeps a rectangular doorway joined continuously to the lower wall, with a restrained inner corner and an open threshold. Use the full notched wall contour; separate jamb and ground strokes can leave steps at the feet. A deliberate architectural arch or plinth remains a design choice, as in Bank. Window and doorway frames of comparable importance use the same structural width. Outline entrance widths may be optically narrower than the solid counters, as documented in the design guide. Preserve the wall silhouette and threshold, and check outline frame separation independently of solid aperture spacing. Storefront retains its right jamb when narrowing the outline door, giving its framed window more clearance.
