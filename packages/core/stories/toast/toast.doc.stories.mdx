...
A toast is a small pop-up notification that provides quick feedback. It’s usually short-lived and displayed in response to a user action or system event that relates to a workflow or application outside the user’s current focus. Use the toast component to notify users of an event that’s occurred in a peripheral application or workflow. A toast should be noticeable without disrupting the user’s experience or requiring immediate action.
