...
For accessibility purposes, we recommend that you add a marker for empty read-only fields via the `emptyReadOnlyMarker` prop. This makes the field easier to be seen amongst other UI elements, indicating its readonly status.
