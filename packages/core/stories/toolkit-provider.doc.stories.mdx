...
By default, the root level `ToolkitProvider` will apply the theme and density classes and the theme mode data attribute to the `<html>` element and any nested `ToolkitProvider` will create a div with the `uitk-provider` class
which the classes and data attributes are added,
if you want the root level `ToolkitProvider` to create a div instead, then pass the value "scope" in the `applyClassesTo`
