...
to the whole of the document rather than just the root `div` that React renders to. Some use cases for this would be to prevent overscroll
or to apply styling to the whole background of the page.
...
```html
<html class="uitk-theme-light uitk-density-medium" lang="en">
  <head>
    <title>Example</title>
  </head>
  <body>
...
You can use the `useTheme` hook to access the value which indicates which theme and mode is currently active. The `useTheme` hook returns an object which contains string values of the currently set theme and mode in scope. You can optionally override these values by providing a theme name and mode string to `ToolkitProvider` via the `theme` and `mode` prop respectively.
