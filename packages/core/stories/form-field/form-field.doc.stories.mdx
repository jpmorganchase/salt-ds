...
When using Form Field with Multiline Input, the `pronounced` prop can be used on the form label and the `fullBorder` prop can be set on Multiline Input to style the field as a question.
...
The Form Field Label can be styled as a question by setting the `pronounced` prop to `true`.
...
- When paired with Multiline Input, pair the pronounced label with a full border.
...
If you are low on screen real-estate, opt for using a [Tooltip](/?path=/docs/documentation-core-tooltip--page) to display your helper text.
