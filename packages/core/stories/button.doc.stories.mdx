...
- A button with only an icon and no text should be self-explanatory, ideally using well known metaphors which are easily related too. If the purpose of the button isn’t clear, a label should be added. If space is a consideration, add a tooltip to the button set to display on hover after a short delay (typically 300ms, `--uitk-delay-perceptible`). The tooltip should add context to the action such as 'Edit this form', 'Print this document', 'Lock document state'. 
