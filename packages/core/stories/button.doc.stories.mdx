...
- If the button contains text and an icon, pass `aria-hidden=”true”` to the icon so its label is not announced by the screen reader.
- If the only content is an icon, pass an `aria-label` to `Button` that describes its purpose.
