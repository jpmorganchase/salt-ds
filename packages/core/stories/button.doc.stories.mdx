...
If the use case requires the user to navigate to a new page or window rather than trigger a function, use a `Link` component instead. The exception to this behaviour is if the navigation is the core action on the page. In this case, a `CTA Button` should be used to trigger the navigation.
