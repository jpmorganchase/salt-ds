...
When using Banner to alert the user `role="alert"` should be applied to `BannerContent`.
...
When using Banner to notify the user `role="status"` should be applied to `BannerContent`.
