...
To make sure the dynamic/conditional Banner is accessible you have to add `role="alert"`, so the announcement will happen instaltly when Banner gets mounted. By default the Banner has a role `banner`.
