...
Link colors are the foreground colors assigned to text styles. These can be: `primary` and `secondary` and `inherit` with `primary` being the same as default.
...
<Canvas>
  <Story id="core-link--inherit-color" />
</Canvas>
