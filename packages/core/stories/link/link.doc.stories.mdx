...
Link colors are the foreground colors assigned to text styles. These are split into two groupings, `primary` and `secondary` with `primary` being the same as default.

Use the `color` prop when you want to change the link color.

#### Primary

<Canvas>
  <Story id="core-link--primary-color" />
</Canvas>

#### Secondary

<Canvas>
  <Story id="core-link--secondary-color" />
</Canvas>
