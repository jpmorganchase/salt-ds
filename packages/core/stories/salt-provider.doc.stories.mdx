...
The UI Toolkit `SaltProvider` is a multi-purpose React Context provider. It allows values for cross-cutting concerns like `Theme` and `Density`
...
application. SaltProviders can also be nested to effect local overrides. Note that because almost all styling effects related to theme and density
...
The `breakpoints` prop on `SaltProvider` can be used to customize the CSS media queries that custom hooks like `useBreakpoints` and `useCurrentBreakpoint` are using internally. These hooks are usually used to enable the responsive props, where you can pass multiple values (one per breakpoint) or a single value that would take effect across all different screen sizes. These props can mostly be found in the layout components for now. Refer to the [FlexLayout](/docs/documentation-layout-flexlayout--page) component documentation for a good example of this.
...
<Canvas>
  <Story name="Simple SaltProvider - Touch Density">
...
<Canvas>
  <Story name="Nested SaltProvider">
    <NestedProviders />
  </Story>
</Canvas>
...
Some browser implementations of `display: contents` are buggy (Safari, for example). Further, your code may rely on CSS query selectors in JavaScript that might be broken if they do not anticipate the additional `div` element inserted by the SaltProvider.
...
Nested `SaltProvider`s will create a div with the `salt-provider` class where the classes and data attributes are added.
