...
Text provides a default highlight color. Use the token `text-background-selected` to indicate a selection of text using a cursor, in both light and dark modes. 

A selection of text should be able to be highlighted with the default highlight color applied, this includes text within input fields.

The highlight function can also be used to highlight images, or other content, alongside text. 
...
There are three emphasis levels across our text styles that can be used to highlight or lowlight body text and headings - the default level with no emphasis, `strong` to emphasise text/content and `small` to de-emphasise text/content. 
...
By using the `maxRows` prop your text component will be truncated and will display the chosen number of rows. Ellipsis will be shown at the end to indicate the text has been truncated. If a value is not set for `maxRows`, the text will wrap.
