...
### --uitkText CSS custom property API

The default styling values for most Text attributes are provided by the theme variables. A smaller number of attributes
use hard-coded values local to Text. Both of these can be overridden, see Theme documentation for detailed usage guidance.

The CSS custom properties below are consumed by Text, but not defined by Text. They can be defined via a container or
