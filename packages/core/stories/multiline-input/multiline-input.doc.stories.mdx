...
Tip: For recommendations on variant choice, see the Forms pattern.
...
Multiline Input shows 3 rows by default. This can be adjusted by setting the `rows` prop. This example demonstrates a Multiline Input with 4 rows.
...
- Bordered styling is recommended when the field uses the same fill colour as the background (i.e., a primary fill colour on a primary background).
...
Multiline Input can show validation states (error, warning, success) with the prop `validationStatus`.
...
Custom adornments can be added at the beginning or end of the Multiline Input field using the `startAdornment` and `endAdornment` props respectively.
...
A character count can be added as a label using ‘endAdornment’ props. This example demonstrates the expected behaviour when the count is exceeded, with the field dynamically displaying an error state and the count continuing to display the number of characters over the limit.
...
Input field width and height should always reflect the amount of content expected in the field. If the content is unlikely to exceed a single line, always use a regular [Input](?path=/docs/documentation-core-input--page) component, rather than a Multiline Input.
...
To be considered accessible within a form, the Multiline Input _must_ be wrapped in a [Form Field](?path=/docs/documentation-core-form-field--page) component, which will provide the field with a visible label, help text and a status message for validation feedback.
