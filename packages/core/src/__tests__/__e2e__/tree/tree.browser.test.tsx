...
function ControlledMultiple({
  ...
  fixed?: boolean;
  onSelectionChange?: ComponentProps<typeof Tree>["onSelectionChange"];
}) {
  const [selected, setSelected] = useState<string[]>(["node1"]);
  return (
    <>
...
describe("Given a Tree", () => {
  ...
  describe("multiple selection", () => {
    ...
    it("supports controlled external selection, Ctrl+A and clearing", async () => {
      const onSelectionChange = vi.fn();
      await renderWithSalt(
        <ControlledMultiple onSelectionChange={onSelectionChange} />,
      );
  ...
  describe("focus management", () => {
    ...
    it("updates the re-entry target as a nested selection becomes visible", async () => {
      await renderWithSalt(
        <>
          <button type="button">Before</button>
          <Tree
      ...
      const expectReentryFocus = async (label: string) => {
        await after.click();
        await userEvent.keyboard("{Shift>}{Tab}{/Shift}");
        await expect.element(treeItem(label)).toHaveFocus();
      };
      ...
      await page.getByRole("button", { name: "Before" }).click();
      await userEvent.tab();
      await expect.element(treeItem("Deep Child 1")).toHaveFocus();

      await expansionIcon("Parent").click();
      await expectTreeItemAbsent("Deep Child 1");
      await expectReentryFocus("Node 1");
      ...
      await expansionIcon("Parent").click();
      await expect.element(treeItem("Deep Child 1")).toBeVisible();
      await expectReentryFocus("Deep Child 1");
    });
  });
