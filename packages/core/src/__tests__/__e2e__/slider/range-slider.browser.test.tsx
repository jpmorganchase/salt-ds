...
describe("Given a Range Slider", () => {
  ...
  it.each(["mouse", "touch"] as const)(
    "drags the second thumb with a %s pointer and stops on release",
    async (pointerType) => {
      const onChange = vi.fn();
      const onChangeEnd = vi.fn();
      await renderWithSalt(
      ...
      expect(onChangeEnd.mock.lastCall?.[1]).toEqual([
        0,
        Number(valueAfterRelease),
      ]);
      ...
      await nextFrame();
      await expect.element(sliders().nth(1)).toHaveValue(valueAfterRelease);
