...
function ControlledPercentInput({
  onChange,
  onNumberChange,
}: Pick<NumberInputProps, "onChange" | "onNumberChange">) {
  ...
  return (
    <NumberInput
      ...
      pattern={(inputValue) => /^[+-]?(\d+(\.\d*)?|\.\d+)%?$/.test(inputValue)}
      format={(inputValue) => `${inputValue}%`}
      parse={(inputValue) =>
        inputValue.length
          ? Number.parseFloat(inputValue.replace(/%/g, ""))
          : null
      }
    />
  );
...
describe("Number Input", () => {
  ...
  it("clamps defaults and typed values to safe integers", async () => {
    const onNumberChange = vi.fn();
    await renderWithSalt(
      <Default
    ...
    await expect.element(input()).toHaveValue(String(Number.MAX_SAFE_INTEGER));

    for (const [typed, clamped] of [
      ["-83491232183712983713791237", Number.MIN_SAFE_INTEGER],
      ["83491232183712983713791237", Number.MAX_SAFE_INTEGER],
    ] as const) {
      await typeValue(typed);
      await userEvent.tab();
      await expect.element(input()).toHaveValue(String(clamped));
      expect(onNumberChange.mock.lastCall?.[1]).toBe(clamped);
    }
  });
  ...
  it("prevents stepping beyond safe integers", async () => {
    ...
    (button("increment").element() as HTMLButtonElement).click();
    ...
    (button("decrement").element() as HTMLButtonElement).click();
    expect(onNumberChange).not.toHaveBeenCalled();
    await expect.element(input()).toHaveValue(String(Number.MIN_SAFE_INTEGER));
  });
  ...
  it.each([
    ...
    "does not step %s with %s beyond its bound",
    async (value, kind, bound) => {
      const onNumberChange = vi.fn();
      await renderWithSalt(
        <Default
          defaultValue={value}
      ...
      await expect.element(button(kind)).toBeDisabled();
      (button(kind).element() as HTMLButtonElement).click();
      await expect.element(input()).toHaveValue(String(value));
      expect(onNumberChange).not.toHaveBeenCalled();
    },
  );
  ...
  it("clamps an out-of-range typed value before the first keyboard step", async () => {
    await renderWithSalt(<Default min={10} max={100} clamp />);
    await typeValue("1000000");
  ...
  describe("formatting", () => {
    it("rejects input that does not match the formatting pattern", async () => {
    ...
    it("reports controlled formatted text while typing and its number on blur", async () => {
      const onChange = vi.fn();
      const onNumberChange = vi.fn();
      await renderWithSalt(
        <ControlledPercentInput
          onChange={onChange}
          onNumberChange={onNumberChange}
        />,
      );
      ...
      await input().click();
      await userEvent.keyboard("{ArrowUp}");
      await expect.element(input()).toHaveValue("16%");
      expect(onChange).toHaveBeenCalledOnce();
      expect(onChange.mock.calls[0]?.[1]).toBe("16%");
      expect(onNumberChange).toHaveBeenCalledOnce();
      expect(onNumberChange.mock.calls[0]?.[1]).toBe(16);

      await userEvent.keyboard("{ArrowDown}");
      await expect.element(input()).toHaveValue("15%");
      expect(onChange).toHaveBeenCalledTimes(2);
      expect(onChange.mock.calls[1]?.[1]).toBe("15%");
      expect(onNumberChange).toHaveBeenCalledTimes(2);
      expect(onNumberChange.mock.calls[1]?.[1]).toBe(15);

      await typeValue("30%");
      await expect.element(input()).toHaveValue("30%");
      expect(onChange.mock.lastCall?.[1]).toBe("30%");
      expect(onNumberChange).toHaveBeenCalledTimes(2);

      const changeCallsBeforeBlur = onChange.mock.calls.length;
      await userEvent.tab();
      await expect.element(input()).toHaveValue("30%");
      expect(onChange).toHaveBeenCalledTimes(changeCallsBeforeBlur);
      expect(onChange.mock.lastCall?.[1]).toBe("30%");
      expect(onNumberChange).toHaveBeenCalledTimes(3);
      expect(onNumberChange.mock.lastCall?.[1]).toBe(30);
    });

    it("reports the typed target and normalized value for uncontrolled formatting", async () => {
      const onChange = vi.fn<NonNullable<NumberInputProps["onChange"]>>(
        (event) => event?.persist(),
      );
      const onNumberChange = vi.fn();
      await renderWithSalt(
      ...
      await expect.element(input()).toHaveValue("30%");
      expect(
        (onChange.mock.lastCall?.[0]?.target as HTMLInputElement | undefined)
          ?.value,
      ).toBe("30%");
      expect(onChange.mock.calls.map(([, value]) => value)).toContain("30");
  ...
  describe("read-only", () => {
    ...
    it("shows its empty marker for a controlled empty value", async () => {
      await renderWithSalt(<ReadOnly value="" readOnly />);
