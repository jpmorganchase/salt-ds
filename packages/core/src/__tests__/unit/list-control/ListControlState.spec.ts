...
describe("findOptionFromSearch", () => {
  it("starts after a late active option and wraps to an earlier match", () => {
    ...
    expect(
      findOptionFromSearch(options, "al", String, options[2].data),
    ).toBe(options[0].data);
  });
  ...
  it("wraps from the final option", () => {
    ...
    expect(
      findOptionFromSearch(options, "c", String, options[2].data),
    ).toBe(options[1].data);
  });
  ...
  it("cycles by first character when the search repeats one character", () => {
    ...
    expect(
      findOptionFromSearch(options, "aa", String, options[0].data),
    ).toBe(options[1].data);
  });
  ...
  it("retains first-value identity semantics for duplicate values", () => {
    ...
    expect(
      findOptionFromSearch(options, "a", String, first.data),
    ).toBe(laterMatch.data);
    expect(
  ...
  it.each([32, 512])(
    "reads a %i-option registry linearly",
    (size) => {
