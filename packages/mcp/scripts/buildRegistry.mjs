...
const distPackageJsonPath = path.join(distPackageRoot, "package.json");
...
try {
  ...
  distEntryPath = path.join(distPackageRoot, distPackageJson.main);
  await fs.access(distEntryPath);
} catch {
  throw new Error(
    "Missing built @salt-ds/semantic-core package output. Run `yarn workspace @salt-ds/semantic-core build` first.",
  );
}
...
try {
  ...
  throw new Error(
    "Missing built @salt-ds/semantic-core buildRegistry output. Run `yarn workspace @salt-ds/semantic-core build` first.",
  );
}
