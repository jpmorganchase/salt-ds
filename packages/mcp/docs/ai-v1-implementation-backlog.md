...
For teams primarily using Salt inside the IDE, the practical workflow order should be:
...
- the consumer AI landing page now leads with:
  - Salt as an AI-native design system product
  - common high-value surfaces
  - design-language guidance before transport details
- the public `salt-ds` skill now has:
  - concrete trigger examples
  - explicit project-context-first guidance
  - clearer "what Salt handles best" framing
- shared skill references now cover:
  - common product surfaces
  - design principles for hierarchy, density, layout ownership, and bounded customization
- a maintainer benchmark reference now exists for:
  - public benchmark archetypes for first-run discoverability
  - AI-ready packaging coherence
  - official MCP setup clarity
  - visible design-system intelligence
...
- the default MCP server now exposes only the six public workflow tools:
...
- convert the new benchmark criteria into checked-in eval fixtures for the priority design-system surfaces
...
- IDE users can start from the active file, selection, feature folder, or attached screen without learning transport details
...
- fold beta, setup, workflow, troubleshooting, install, and conventions guidance into [`site/docs/getting-started/ai.mdx`](../../../site/docs/getting-started/ai.mdx)
...
- shorten [`../../../skills/salt-ds/SKILL.md`](../../../skills/salt-ds/SKILL.md)
...
The default beta MCP surface is now the six public workflow tools only.
...
- update setup and conventions guidance to reflect the split
...
- keep setup, workflows, troubleshooting, install source, and conventions guidance in one page
...
- for IDE-first teams, make `review` and `upgrade` the strongest default task shapes before broadening `create`
...
- keep the AI landing page and `salt-ds` front section aligned with the benchmark-backed product story
  - value before transport
  - project context before guessing
  - common surface guidance visible to consumers
...
- story/example-derived starter templates for the priority patterns
  - keep docs-owned scaffold semantics, but replace the remaining pattern-specific fallback JSX bodies with extracted canonical story or docs-example starter sources when the supporting examples are ready
