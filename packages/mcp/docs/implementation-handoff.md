...
| 10 | **0.7** close the 0.6 coverage gap list (upstream fixes) | Opus 4.7 (medium) | Agent | 3 hr | Medium | ⚠️ Partial (was ✅ Done at rev 11, flipped back at rev 13 after the 2026-06-12 failures audit). `packages/mcp/src/__tests__/registryCoverage.spec.ts` re-fails on a freshly-built registry artifact: `SaltProviderNext is present as a first-class registry entity (roadmap F1)` reports a 2-entry gap (no component or pattern record exists for `SaltProviderNext`, and no canonical example in `examples.json` references it). The rev-11 5/5 PASS reading was based on a stale `packages/mcp/generated/` directory (the path is **gitignored** — the test suite regenerates it on first run, and the regenerator does not extract `SaltProviderNext`). The partial PR 10 catch-up commit `3758dbb5b` only extracted `SaltProviderNext` props as a side artifact; it did not wire the entity into `buildRegistryComponents.ts` as a first-class component record. **Real work still required:** a build-pipeline change in `packages/semantic-core/src/build/buildRegistryComponents.ts` (or a peer module) that emits a `SaltProviderNext` component record with the brand-prop defaults so `get_salt_entity('SaltProviderNext')` resolves without forcing the model to read `node_modules`. This is the F1 / M9 watchdog the original task 0.7 was meant to close. Other 4 assertions in the spec (28 patterns without `composition_contract`, 24 foundations without canonical examples, etc.) continue to pass — only the `SaltProviderNext` first-class-entity gap remains. |
