...
Release 1 is signed off for controlled rollout as of April 21, 2026.
...
- [x] `GATE-11` One real external MCP host validates manifest access, top-level contract branching, and weak-context failure behavior.
      Acceptance artifact: [`./release-1-release-evidence.md`](./release-1-release-evidence.md)
      Gate: external host validation completed
      Evidence: IntelliJ AI Assistant read `salt://capabilities/manifest`, reported compact contract version `v3`, preserved exact-name and weak-context safety behavior, and exposed the remaining host orchestration and first-pass routing issues now tracked in [`./post-release-1-follow-up-backlog.md`](./post-release-1-follow-up-backlog.md).
