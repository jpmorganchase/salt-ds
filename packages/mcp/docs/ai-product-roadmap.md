...
For teams primarily operating inside the IDE, the preferred workflow order should be:
...
For the concrete productionization checklist behind extension packs and layered policy, see [`./extension-productionization-checklist.md`](./extension-productionization-checklist.md).
...
- benchmark against real design-system products
  - use public benchmark archetypes for first-run discoverability, AI-ready packaging coherence, and official MCP setup clarity
...
- expose more design-system judgment, not only workflow control
  - common surfaces, hierarchy, density, layout ownership, and bounded customization should be visible in the product, not only implied in internal reasoning
...
- for IDE-first usage, make `review` and `upgrade` the strongest default task modes before broadening `create`
