...
export interface ComponentStarterCodeOptions {
  /**
   * Full attached examples are useful for explicit example retrieval, but
   * entity-reference starter guidance should stay small enough to copy safely.
   */
  include_supporting_example?: boolean;
}
...
export function createComponentStarterCode(
  component: ComponentRecord,
  options: ComponentStarterCodeOptions = {},
): StarterCodeSnippet[] {
  ...
  if (example && options.include_supporting_example !== false) {
...
export function createRecipeStarterCode(input: {
  ...
  prefer_supporting_example_as_starter?: boolean;
  include_supporting_example?: boolean;
}): StarterCodeSnippet[] {
  const structuredScaffold = shouldUseStructuredScaffold(input.starter_scaffold)
    ? input.starter_scaffold
    : null;
  const usableSupportingExample = hasUsableSupportingExample(
    input.supporting_example,
  )
    ? input.supporting_example
    : null;
  const exampleBackedStarter =
    input.prefer_supporting_example_as_starter === false
      ? null
      : usableSupportingExample;
  ...
  const canBuildGenericStarter =
    allowGenericComponentStarter && input.components.length > 0;
  if (!exampleBackedStarter && !fallbackTemplate && !canBuildGenericStarter) {
    return [];
  }
  ...
  if (
    input.include_supporting_example !== false &&
