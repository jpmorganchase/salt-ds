 ...
 * own.
 *
 * Internal filters are applied to every name in the batch. The public
 * `get_salt_reference` surface intentionally exposes only exact-name routing,
 * an optional entity-family discriminator, and bounded payload includes.
 */
...
const MAX_INCLUDED_EXAMPLES_PER_BATCH = 3;
const MAX_INCLUDED_EXAMPLES_PER_ENTITY = 1;
const MAX_INCLUDED_EXAMPLE_CODE_BYTES_PER_BATCH = 24 * 1024;

function isRecord(value: unknown): value is Record<string, unknown> {
  return Boolean(value) && typeof value === "object" && !Array.isArray(value);
}

function getExampleDeduplicationKey(value: unknown): string | null {
  if (!isRecord(value)) {
    return null;
  }
  ...
  return id || sourceUrl ? `ref:${id}:${sourceUrl}` : null;
}

function deduplicateBatchExamples(
  rows: GetSaltEntitiesRow[],
): GetSaltEntitiesRow[] {
  const seenExamples = new Set<string>();
  let includedExamples = 0;
  let includedCodeBytes = 0;

  return rows.map((row) => {
    const entity = row.result.entity;
    const examples = isRecord(entity) ? entity.examples : null;
    ...
    for (const example of examples) {
      const key = getExampleDeduplicationKey(example);
      if (
        (key !== null && seenExamples.has(key)) ||
        includedExamples >= MAX_INCLUDED_EXAMPLES_PER_BATCH ||
        included.length >= MAX_INCLUDED_EXAMPLES_PER_ENTITY
      ) {
        omitted += 1;
        continue;
      }
      ...
      const exampleRecord = isRecord(example) ? example : null;
      const code =
        exampleRecord && typeof exampleRecord.code === "string"
          ? exampleRecord.code
          : null;
      const codeBytes = code ? Buffer.byteLength(code, "utf8") : 0;
      if (
        code &&
        exampleRecord &&
        includedCodeBytes + codeBytes >
          MAX_INCLUDED_EXAMPLE_CODE_BYTES_PER_BATCH
      ) {
        const { code: _code, ...metadata } = exampleRecord;
        included.push({
          ...metadata,
        ...
        included.push(example);
        includedCodeBytes += codeBytes;
      }
      includedExamples += 1;
    }

    if (omitted === 0 && codeOmitted === 0) {
      return row;
    }
    ...
    return {
      ...
      result: {
        ...
        entity: {
          ...entity,
          examples: included,
          ...(omitted > 0 ? { examples_omitted_in_batch: omitted } : {}),
          ...(codeOmitted > 0
            ? { example_codes_omitted_in_batch: codeOmitted }
            : {}),
        },
      },
    };
...
export function getSaltEntities(
  ...
  const rows = deduplicateBatchExamples(
  ...
  if (foundCount === rows.length) {
    ...
    nextStep =
      "Re-check the Salt entity names against the registry, or use create_salt_ui when the request is still a broad design intent.";
  } else {
    status = "partial";
    ...
    nextStep = `Inspect the per-name rows for ${unresolvedNames
      ...
      .join(
        ", ",
      )} (each row carries its own did_you_mean and ambiguity payload), then retry just the unresolved names or use create_salt_ui for the broader intent.`;
  }
