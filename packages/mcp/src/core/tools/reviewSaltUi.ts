...
import {
  type CanonicalComponentExportOwner,
  type CanonicalPackageExportSource,
  findCanonicalComponentExportOwner,
  findCanonicalPackageExportEvidence,
...
export interface ReviewSaltUiInternalInput {
  approved_wrappers?: readonly ReviewApprovedWrapper[];
}

export interface ReviewApprovedWrapper {
  wraps: string;
  import: {
    from: string;
    name: string;
...
export interface ReviewSaltUiResult {
  ...
  artifact_verification: ReviewArtifactVerification;
...
export interface ReviewArtifactImportVerification {
  package: string;
  imported: string;
  resolved_entity: string | null;
  resolution:
    | "component_registry"
    | "canonical_example_export"
    | "canonical_package_export"
    | null;
...
export interface ReviewArtifactVerification {
  status: "complete" | "incomplete" | "not_applicable";
  component_imports: ReviewArtifactImportVerification[];
}
...
function isComponentLikeImportName(value: string): boolean {
  return /^[A-Z][A-Za-z0-9]*[a-z][A-Za-z0-9]*$/.test(value);
}

function artifactImportIssueId(packageName: string, imported: string): string {
  return `artifact-import.unsupported.${packageName
    .replace(/^@/, "")
    .replace(/[^a-zA-Z0-9]+/g, "-")
    .toLowerCase()}.${imported.replace(/[^a-zA-Z0-9]+/g, "-").toLowerCase()}`;
}

function buildCanonicalExampleExportEvidenceRef(input: {
  registry: SaltRegistry;
  imported: string;
  owner: CanonicalComponentExportOwner;
}): SaltEvidenceRef {
  return {
    contract: SALT_EVIDENCE_REF_CONTRACT,
    id: `${input.owner.component.id}.${input.imported
      .replace(/[^a-zA-Z0-9]+/g, "-")
      .toLowerCase()}.canonical-example-export.review-ref`,
    source_kind: "example",
    claim_kind: "import",
    registry: {
      entity_type: "component",
      entity_id: input.owner.component.id,
      entity_name: input.owner.component.name,
      field_path: `examples.${input.owner.example.id}.code`,
      registry_version: input.registry.version,
    },
    source: {
      url: input.owner.example.source_url,
    },
    confidence: "high",
    verified_at: input.owner.component.last_verified_at,
    note: `Canonical ${input.owner.component.name} example imports ${input.imported} from ${input.owner.component.package.name}.`,
  };
}

function buildCanonicalPackageExportEvidenceRef(input: {
  registry: SaltRegistry;
  packageName: string;
  imported: string;
  source: CanonicalPackageExportSource;
}): SaltEvidenceRef {
  return {
    contract: SALT_EVIDENCE_REF_CONTRACT,
    id: `${input.source.entity_id}.${input.imported
      .replace(/[^a-zA-Z0-9]+/g, "-")
      .toLowerCase()}.canonical-package-export.review-ref`,
    source_kind: "example",
    claim_kind: "import",
    registry: {
      entity_type: input.source.entity_type,
      entity_id: input.source.entity_id,
      entity_name: input.source.entity_name,
      field_path: `examples.${input.source.example_id}.code`,
      registry_version: input.registry.version,
    },
    source: { url: input.source.source_url },
    confidence: "high",
    verified_at: input.source.verified_at,
    note: `Canonical ${input.source.entity_name} example imports ${input.imported} from ${input.packageName}.`,
  };
}

function buildArtifactImportVerification(
  registry: SaltRegistry,
  ...
  const componentImports = new Map<
    string,
    { packageName: string; imported: string }
  >();
  const addComponentImport = (packageName: string, imported: string) => {
    // Icon exports have their own registry-backed validator. Keeping them out
    // of this component pass avoids reporting every valid icon as an unknown
    // component.
    if (packageName === "@salt-ds/icons") {
      return;
    }

    const key = `${packageName}\u0000${imported}`;
    componentImports.set(key, { packageName, imported });
  };

  for (const imported of analysis.saltImports) {
    if (imported.typeOnly || imported.imported === "*") {
      continue;
    }
    ...
    if (
      ...
      isComponentLikeImportName(imported.imported)
    ) {
      addComponentImport(imported.packageName, imported.imported);
    }
  }
  ...
  const imports = [...componentImports.values()]
    .sort((left, right) =>
      `${left.packageName}:${left.imported}`.localeCompare(
        `${right.packageName}:${right.imported}`,
      ),
    )
    .map<ReviewArtifactImportVerification>(({ packageName, imported }) => {
      const component = findComponentByPackageAndLookupName(
        registry,
        packageName,
        imported,
      );
      const canonicalExportOwner = component
        ? null
        : findCanonicalComponentExportOwner(registry, packageName, imported);
      const canonicalPackageExport =
        component || canonicalExportOwner
          ? null
          : findCanonicalPackageExportEvidence(registry, packageName, imported);
      const resolvedComponent = component ?? canonicalExportOwner?.component;
      const id = artifactImportIssueId(packageName, imported);

      if (!resolvedComponent && !canonicalPackageExport) {
        const message = `Imported Salt component ${imported} from ${packageName} could not be resolved to a Salt registry component record.`;
        ...
        issues.push({
          ...
          severity: "error",
          title: `Salt component import ${imported} is unverified`,
          message,
          evidence: buildEvidence(
            `Reviewed artifact imports ${imported} from ${packageName}`,
            1,
          ),
          canonical_source: null,
          suggested_fix:
            "Confirm the canonical exported component name with get_salt_reference, then replace or remove the unsupported import before shipping.",
          confidence: 0.99,
          source_urls: [],
          evidence_refs: [
            buildWorkflowInputExpectedTargetEvidenceRef({
              ...
              note: `The reviewed code imports ${imported} from ${packageName}.`,
            }),
          ],
          matches: 1,
          fix_hints: {
            ...
            related_components: [],
            extra_steps: [
              `Look up ${imported} with get_salt_reference before editing the import.`,
            ],
          },
        });
      ...
      if (canonicalPackageExport && !resolvedComponent) {
        const sources = canonicalPackageExport.sources.slice(0, 4);
        return {
          package: packageName,
          imported,
          resolved_entity: imported,
          resolution: "canonical_package_export",
          status: "verified",
          source_urls: unique(sources.map((source) => source.source_url)),
          evidence_refs: sources.map((source) =>
            buildCanonicalPackageExportEvidenceRef({
              registry,
              packageName,
              imported,
              source,
      ...
      if (!resolvedComponent) {
        throw new Error(
          `Canonical import verification for ${packageName}/${imported} did not resolve an evidence source.`,
        );
      }

      const sourceUrls = unique([
        ...componentDocUrls(registry, resolvedComponent.name, [
          "overview",
          "usage",
          "examples",
        ]),
        ...(canonicalExportOwner?.source_urls ?? []),
      ]);
      return {
        package: packageName,
        imported,
        resolved_entity: resolvedComponent.name,
        resolution: component
          ? "component_registry"
          : "canonical_example_export",
        status: "verified",
        source_urls: sourceUrls,
        evidence_refs: canonicalExportOwner
          ? [
              buildCanonicalExampleExportEvidenceRef({
                registry,
                imported,
                owner: canonicalExportOwner,
              }),
            ]
          : [
              buildComponentRegistryEvidenceRef({
                ...
                id_suffix: "reviewed-artifact-import",
              }),
            ],
      };
    });
  ...
  return {
    artifact_verification: {
      status:
        imports.length === 0
          ? "not_applicable"
          : imports.every((entry) => entry.status === "verified")
            ? "complete"
            : "incomplete",
...
function buildExpectedPatternUnsupportedIssue(
  ...
  const reasonMessage =
    reason === "missing_pattern_record"
      ? "No matching Salt registry pattern record was found."
      : "The matching Salt registry pattern record has no supported source-backed pattern validation rules.";
  ...
  return {
    ...
    message: `The workflow expected ${target.name}, but the Salt MCP cannot validate that pattern implementation from registry evidence. ${reasonMessage}`,
    evidence: buildEvidence(
...
function hasSemanticRegionMarker(code: string, marker: string): boolean {
  if (hasTextPresenceMarker(code, marker)) {
    return true;
  }

  const normalizedMarker = normalizeTextPresence(marker);
  const semanticTag = normalizedMarker.includes("header")
    ? "header"
    : normalizedMarker.includes("navigationpane") ||
        normalizedMarker.includes("fixedpanel") ||
        normalizedMarker.includes("sidebar")
      ? "aside"
      : normalizedMarker.includes("mainbody") ||
          normalizedMarker.includes("maincontent") ||
          normalizedMarker.includes("primarycontent") ||
          normalizedMarker.includes("contentarea") ||
          normalizedMarker.includes("contentcontext")
        ? "main"
        : null;

  return semanticTag
    ? new RegExp(`<${semanticTag}(?=[\\s>])`, "i").test(code)
    : false;
}
...
function buildExpectedPatternIssues(input: {
  ...
  importedModuleKeys: Set<string>;
  approvedWrappers: readonly ReviewApprovedWrapper[];
  ...
  if (!pattern) {
    return {
      ...
      missing_data: [
        `Expected pattern target '${input.target.name}' was not structurally validated because no matching Salt registry pattern record was found.`,
      ],
  ...
  if (
    ...
    if (unsupportedRuleKinds.length > 0) {
      return {
        issues: [],
        missing_data: [],
        unsupported_rule_kinds: unsupportedRuleKinds,
      };
    }
  ...
  for (const rule of supportedImportRules) {
    ...
    if (
      ...
      (input.importedSaltImportKeys.has(
        ...
        isCanonicalComponentSatisfiedByApprovedWrapper({
          registry: input.registry,
          names: [importMatch.name],
          approvedWrappers: input.approvedWrappers,
          importedModuleKeys: input.importedModuleKeys,
        }))
    ) {
      continue;
    }
  ...
  for (const rule of supportedRegionRules) {
    if (
      ...
      hasSemanticRegionMarker(input.code, rule.match.text)
    ) {
      continue;
    }
...
function buildModuleImportKey(moduleName: string, importName: string): string {
  return `${moduleName}\u0000${importName}`;
}

function collectImportedModuleKeys(analysis: SaltCodeAnalysis): Set<string> {
  ...
  traverseAst(analysis.ast, {
    ImportDeclaration(path) {
      ...
      for (const specifier of path.node.specifiers) {
        ...
        importedKeys.add(buildModuleImportKey(moduleName, importName));
      }
    },
  });
...
function isCanonicalComponentSatisfiedByApprovedWrapper(input: {
  registry: SaltRegistry;
  names: string[];
  approvedWrappers: readonly ReviewApprovedWrapper[];
  importedModuleKeys: Set<string>;
  ...
  const targetKeys = new Set(
    input.names.flatMap(
      (name) => componentImportKeys(input.registry, name).keys,
    ),
  );

  return input.approvedWrappers.some(
    (wrapper) =>
      wrapper.import !== null &&
      targetKeys.has(normalizeTargetName(wrapper.wraps)) &&
      input.importedModuleKeys.has(
        buildModuleImportKey(wrapper.import.from, wrapper.import.name),
      ),
  );
}
...
function buildExpectedTargetValidation(
  ...
  if (!expectedTargets || !input.analysis) {
    return { issues: [], missing_data: [], unsupported_rule_kinds: [] };
  }

  const issues: ValidationIssue[] = [];
  const missingData: string[] = [];
  const unsupportedRuleKinds: string[] = [];
  ...
  const importedModuleKeys = collectImportedModuleKeys(input.analysis);
  ...
  if (expectedPatternTargets.length > 0) {
    patternRulePack = getRegistryPatternValidationRulePack(registry);
  ...
  for (const target of expectedPatternTargets) {
    ...
    unsupportedRuleKinds.push(...patternValidation.unsupported_rule_kinds);
  }
  ...
  for (const target of collectExpectedComponentTargets(expectedTargets)) {
    ...
    for (const unresolvedComponent of unresolvedComponents) {
      missingData.push(
        `Expected component target '${unresolvedComponent}' could not be resolved to a Salt registry component record.`,
      );
    }
  }
  ...
  return {
    issues,
    missing_data: unique(missingData),
    unsupported_rule_kinds: unique(unsupportedRuleKinds),
  };
}
...
export function reviewSaltUi(
  ...
  internalInput: ReviewSaltUiInternalInput = {},
): ReviewSaltUiResult {
  ...
  const contextualValidation = buildExpectedTargetValidation(registry, {
    ...
    approvedWrappers: internalInput.approved_wrappers ?? [],
    analysis,
    code: input.code,
  });
  const iconValidation = buildSaltIconImportIssues(registry, analysis);
  const artifactImportValidation = buildArtifactImportVerification(
    registry,
    analysis,
  );
  ...
  // Fix recipes score migrations against each visible issue. Build the full
  // supported candidate set once so a relevant migration is not lost merely
  // because it sorts after the review's visible migration limit.
  const migrationCandidates = suggestMigration(registry, {
    code: input.code,
  ...
  const visibleMigrationRecords = migrationCandidates.migrations.slice(
    0,
    maxIssues,
  );
  const migrations = {
    ...migrationCandidates,
    migrations: visibleMigrationRecords,
    source_urls: unique(
      visibleMigrationRecords.flatMap((migration) => migration.source_urls),
    ),
  };
  const fixes = recommendFixRecipes(
    registry,
    {
      code: input.code,
      framework: input.framework,
    ...
    },
    {
      ...
      migrations: migrationCandidates.migrations,
    },
  );
  ...
  return {
    ...
    artifact_verification: artifactImportValidation.artifact_verification,
    unsupported_rule_kinds: contextualValidation.unsupported_rule_kinds,
