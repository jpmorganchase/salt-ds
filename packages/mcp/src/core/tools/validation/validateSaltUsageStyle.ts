...
function isZeroSizeLiteralString(raw: string): boolean {
  return /^-?0(?:\.0+)?(?:px|rem|em|vh|vw)?$/i.test(raw.trim());
}
...
function isHardcodedSizeLiteral(value: t.Expression): boolean {
  ...
  if (t.isStringLiteral(value)) {
    ...
    return (
      !isZeroSizeLiteralString(raw) &&
      /^-?\d+(\.\d+)?(px|rem|em|vh|vw)$/i.test(raw)
    );
  }
  if (
...
function isHardcodedSizeString(raw: string): boolean {
  ...
  if (isZeroSizeLiteralString(normalized)) {
    return false;
  }
