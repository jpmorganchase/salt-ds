...
export function getTokenNextStep(
  token: Pick<TokenRecord, "deprecated" | "name" | "policy"> | undefined,
): string {
  if (!token) {
  ...
  if (token.deprecated) {
    return `${token.name} is deprecated. Do not use it in new code. This record has no source-backed replacement; consult authoritative Salt migration guidance before choosing one.`;
  }
