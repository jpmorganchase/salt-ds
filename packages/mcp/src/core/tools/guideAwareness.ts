  ...
  return Boolean(
    value &&
      ...
      (!("componentNames" in (value as object)) ||
        Array.isArray(
          (value as { componentNames?: unknown }).componentNames,
        )) &&
...
export function mergeGuideContexts(
  ...
  return normalizeGuideContext({
    componentNames: contexts.flatMap(
      (context) => context?.componentNames ?? [],
    ),
...
export function getRelevantGuidesForRecords(
  ...
  if (embeddedGuides.length > 0) {
    return embeddedGuides.slice(
      0,
      Math.max(1, Math.min(options?.top_k ?? 4, 10)),
    );
  }
