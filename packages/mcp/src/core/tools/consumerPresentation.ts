  ...
  return [
    {
      ...
      reason: `Inspect the canonical Salt foundation record for ${title.toLowerCase()}.`,
      args: {
        names: [title],
...
export function getComponentCaveats(component: ComponentRecord): string[] {
  ...
  if (
    ...
    if (deprecatedPropNames.length > 0 && deprecatedPropNames.length <= 5) {
      caveats.push(
        `${component.name} has deprecated props (${deprecatedPropNames.join(", ")}); review deprecation and migration guidance before adopting new usage.`,
      );
    } else {
      caveats.push(
        `${component.name} includes deprecated APIs; review deprecation and migration guidance before adopting new usage.`,
      );
    }
  }
...
export function getComponentSuggestedFollowUps(
  ...
  if (alternatives[0]) {
    followUps.push({
      ...
      args: {
        query: `Compare ${component.name} with ${alternatives[0]}`,
...
export function getPatternSuggestedFollowUps(
  ...
  if (pattern.related_patterns[0]) {
    followUps.push({
      ...
      args: {
        query: `Compare ${pattern.name} with ${pattern.related_patterns[0]}`,
