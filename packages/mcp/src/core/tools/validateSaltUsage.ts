...
import {
  ...
  hasNavigationComponentCandidate,
...
export function validateSaltUsage(
  ...
  const fidelity = input.fidelity ?? "production";
  ...
  let verticalNavigationTriggerWithoutContentMatches = 0;
  let verticalNavigationPlaceholderHrefMatches = 0;
  ...
  const isDeclaredCompoundComponentPair = (
    outerImported: ImportedSaltSymbol,
    innerImported: ImportedSaltSymbol,
    component: SaltRegistry["components"][number],
    innerComponent: SaltRegistry["components"][number],
  ): boolean =>
    outerImported.packageName === innerImported.packageName &&
    outerImported.imported !== innerImported.imported &&
    Boolean(
      component.sub_components?.some(
        (subComponent) =>
          subComponent.export_name === outerImported.imported ||
          subComponent.export_name === innerImported.imported,
      ) ||
        component.alternatives.some(
          (alternative) =>
            /\bcontains?\b/iu.test(alternative.reason) &&
            normalizeComponentKey(alternative.use) ===
              normalizeComponentKey(innerComponent.name),
        ),
    );

  const isImportedSaltName = (
    imported: ImportedSaltSymbol | null,
    packageName: string,
    name: string,
  ): boolean =>
    imported?.packageName === packageName && imported.imported === name;

  const hasVerticalNavigationItemContentAncestor = (
  ...
  traverseAst(ast, {
    JSXOpeningElement(path) {
      ...
      if (
        component &&
        hasNavigationTargetAttribute(attributes) &&
        !hasNavigationComponentCandidate(component)
      ) {
    ...
    JSXElement(path) {
      ...
      if (
        ...
        if (!hasVerticalNavigationItemContentAncestor(path)) {
          verticalNavigationTriggerWithoutContentMatches += 1;
        }
        const href = getStaticStringAttributeValue(
          attributes.find(
            (attribute) => getJsxAttributeName(attribute) === "href",
          ),
        );
        if (href?.trim() === "#" && fidelity === "production") {
          verticalNavigationPlaceholderHrefMatches += 1;
        }
      }
      if (component) {
          ...
          (candidate) => {
            ...
            return Boolean(
              nestedComponent &&
                ...
                !isDeclaredCompoundComponentPair(
                  imported,
                  candidate,
                  component,
                  nestedComponent,
  ...
  if (verticalNavigationTriggerWithoutContentMatches > 0) {
    ...
    addIssue({
      ...
      rule: "vertical-navigation-trigger-requires-item-content",
      severity: "error",
      title: "VerticalNavigationItemContent is missing",
      message:
        "VerticalNavigationItemTrigger must be wrapped in VerticalNavigationItemContent so VerticalNavigation item state styling is applied.",
      evidence: buildEvidence(
        "Detected VerticalNavigationItemTrigger without a VerticalNavigationItemContent ancestor",
        verticalNavigationTriggerWithoutContentMatches,
      ),
      canonical_source: sourceUrls[0] ?? null,
      suggested_fix:
        "Wrap each VerticalNavigationItemTrigger with VerticalNavigationItemContent inside VerticalNavigationItem.",
      confidence: 0.94,
        ...
        ...(component
          ? [
              buildComponentRegistryEvidenceRef({
                ...
                id_suffix: "vertical-navigation-item-content",
              }),
            ]
          : []),
        {
      ...
      matches: verticalNavigationTriggerWithoutContentMatches,
      fix_hints: {
  ...
  if (verticalNavigationPlaceholderHrefMatches > 0) {
    ...
    const sourceUrls = component
      ? componentDocUrls(registry, component.name, ["usage", "accessibility"])
      : [];

    addIssue({
      id: "navigation.fake-href",
      category: "accessibility",
      rule: "vertical-navigation-trigger-requires-real-destination",
      severity: "error",
      title: "Vertical navigation uses a placeholder destination",
      message:
        'VerticalNavigationItemTrigger must not ship with href="#". Use a real destination, render a router link, or omit href so an in-page action renders as a button.',
      evidence: buildEvidence(
        'Detected VerticalNavigationItemTrigger with placeholder href="#"',
        verticalNavigationPlaceholderHrefMatches,
      ),
      canonical_source: sourceUrls[0] ?? null,
      suggested_fix:
        "Replace the placeholder href with a real route, or remove href for an in-page action.",
      confidence: 0.98,
      source_urls: sourceUrls,
      evidence_refs: [
        ...(component
          ? [
              buildComponentRegistryEvidenceRef({
                ...
                field_path: "related_docs.usage",
                id_suffix: "vertical-navigation-real-destination",
              }),
            ]
          : []),
        {
      ...
      matches: verticalNavigationPlaceholderHrefMatches,
      fix_hints: {
        related_components: ["Vertical navigation"],
        guide_lookups: [],
        extra_steps: [
          "Confirm whether the item navigates or performs an in-page action before choosing link or button semantics.",
        ],
      },
    });
  }
