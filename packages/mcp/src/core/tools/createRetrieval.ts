...
const NON_DISCRIMINATING_PATTERN_ANCHOR_TERMS = new Set(["salt", "pattern"]);

function hasCandidateSpecificPatternAnchor(
  candidate: Omit<CreateRetrievalCandidate, "confidence" | "total_score">,
): boolean {
  if (
  ...
  return candidate.evidence.some(
    (entry) =>
      ...
      (entry.source_kind === "canonical_name" ||
        entry.source_kind === "alias") &&
      entry.matched_terms.some(
        (term) => !NON_DISCRIMINATING_PATTERN_ANCHOR_TERMS.has(term),
      ),
  );
}
...
export function retrieveCreateCandidates(
  ...
  const candidates = [...candidateMap.values()]
    .map((candidate) => {
      ...
      const candidatePatternIntentStrong =
        patternIntentStrong && hasCandidateSpecificPatternAnchor(candidate);

      if (candidate.entity_type === "pattern") {
        if (candidatePatternIntentStrong) {
