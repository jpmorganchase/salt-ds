...
interface RecommendFixRecipesPreparedInput {
  analysis?: SaltCodeAnalysis;
  validation?: ValidateSaltUsageResult;
  migrations?: SuggestMigrationResult["migrations"];
}
...
export function recommendFixRecipes(
  ...
  prepared: RecommendFixRecipesPreparedInput = {},
): RecommendFixRecipesResult {
  let analysis = prepared.analysis;
  if (!analysis && input.code.trim().length > 0) {
    try {
  ...
  const usedMigrations = recipes.flatMap((recipe) => recipe.migrations ?? []);
  const sourceUrls = unique([
    ...validation.issues.flatMap((issue) => issue.source_urls),
    ...usedMigrations.flatMap((migration) => migration.source_urls),
  ]);
