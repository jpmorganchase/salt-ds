...
const STRICT_QUERY_ALIGNMENT_THRESHOLD = 12;
...
export function scorePatternAlignment(
  ...
  for (const intent of intents) {
    ...
    const queryDrivenMatch =
      categoryMatch &&
      overlapScore >= STRICT_QUERY_ALIGNMENT_THRESHOLD &&
      queryScore.matched_terms.length >= 2;

    if (!categoryMatch || (!semanticMatch && !queryDrivenMatch)) {
      continue;
    }
      ...
      matchReasons: mergeUniqueStrings(
        ...
        [
          semanticMatch
            ...
            : "grouped_intent_query_alignment",
        ],
      ),
    };
...
export function getRecipeNextStep(
  ...
  return "Broaden the query in create_salt_ui, or resolve an exact pattern name with get_salt_reference.";
}
...
export function toRecipeStarterCode(
  ...
  const starterScaffold =
    recipe?.starter_scaffold && typeof recipe.starter_scaffold === "object"
      ? (recipe.starter_scaffold as {
          fidelity?: unknown;
          source_urls?: unknown;
          example_source_urls?: unknown;
          semantics?: unknown;
          regions?: unknown;
          required_regions?: unknown;
          optional_regions?: unknown;
          build_around?: unknown;
          preserve_constraints?: unknown;
          template?: unknown;
        })
      : null;
  const starterScaffoldSemantics =
    starterScaffold?.semantics && typeof starterScaffold.semantics === "object"
      ? (starterScaffold.semantics as {
          regions?: unknown;
          required_regions?: unknown;
          optional_regions?: unknown;
          build_around?: unknown;
  ...
  return createRecipeStarterCode({
    ...
    components: Array.isArray(recipe.components)
      ...
      : [],
    starter_scaffold: starterScaffold
      ? {
          fidelity:
            starterScaffold.fidelity === "draft"
              ? "draft"
              : starterScaffold.fidelity === "hybrid"
                ? "hybrid"
                : "canonical",
          source_urls: Array.isArray(starterScaffold.source_urls)
            ? starterScaffold.source_urls.map((value: unknown) => String(value))
            : [],
          example_source_urls: Array.isArray(
            starterScaffold.example_source_urls,
          )
            ? starterScaffold.example_source_urls.map((value: unknown) =>
                String(value),
              )
            : [],
          semantics: {
            regions: Array.isArray(starterScaffoldSemantics?.regions)
              ? starterScaffoldSemantics.regions.map((value: unknown) =>
                  String(value),
                )
              : [],
            required_regions: Array.isArray(
              starterScaffoldSemantics?.required_regions,
            )
              ? starterScaffoldSemantics.required_regions.map(
                  (value: unknown) => String(value),
                )
              : [],
            optional_regions: Array.isArray(
              starterScaffoldSemantics?.optional_regions,
            )
              ? starterScaffoldSemantics.optional_regions.map(
                  (value: unknown) => String(value),
                )
              : [],
            build_around: Array.isArray(starterScaffoldSemantics?.build_around)
              ? starterScaffoldSemantics.build_around.map((value: unknown) =>
                  String(value),
                )
              : [],
            preserve_constraints: Array.isArray(
              starterScaffoldSemantics?.preserve_constraints,
            )
              ? starterScaffoldSemantics.preserve_constraints.map(
                  (value: unknown) => String(value),
                )
              : [],
          },
          template:
            starterScaffold.template &&
            typeof starterScaffold.template === "object"
              ? {
                  kind: "fallback-template" as const,
                  imports: Array.isArray(
                    (starterScaffold.template as Record<string, unknown>)
                      .imports,
                  )
                    ? (
                        (starterScaffold.template as Record<string, unknown>)
                    ...
                    : [],
                  jsx_lines: Array.isArray(
                    (starterScaffold.template as Record<string, unknown>)
                      .jsx_lines,
                  )
                    ? (
                        (starterScaffold.template as Record<string, unknown>)
                          .jsx_lines as unknown[]
                      ).map((value: unknown) => String(value))
                    : [],
                  notes: Array.isArray(
                    (starterScaffold.template as Record<string, unknown>).notes,
                  )
                    ? (
                        (starterScaffold.template as Record<string, unknown>)
                          .notes as unknown[]
