...
function tokenMatches(left: string, right: string): boolean {
  return (
    left === right ||
    normalizeComparableToken(left) === normalizeComparableToken(right)
  );
}

function isIgnoredToken(token: string, ignoredTokens: Set<string>): boolean {
  return [...ignoredTokens].some((ignoredToken) =>
    tokenMatches(token, ignoredToken),
  );
}
...
function overlapsAnyMeaningfulToken(
  ...
  return left.some(
    (leftToken) =>
      !isIgnoredToken(leftToken, ignoredTokens) &&
      right.some(
        (rightToken) =>
          !isIgnoredToken(rightToken, ignoredTokens) &&
          tokenMatches(leftToken, rightToken),
      ),
  );
}
...
function inferPreferredPatternsForSlot(input: {
  ...
  const primaryPatternTokens = new Set(tokenize(input.pattern.name));
      ...
      .filter((candidate) => {
        ...
        return (
          overlapsAnyMeaningfulToken(
            input.slotTokens,
            candidateTokens,
            primaryPatternTokens,
          ) &&
          overlapsAnyMeaningfulToken(
            docsBackedPatternTokens,
            candidateTokens,
            primaryPatternTokens,
          )
        );
