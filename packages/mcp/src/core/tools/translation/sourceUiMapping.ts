...
import { createSaltUi, type SaltSolutionType } from "../createSaltUi.js";
...
function extractDecisionRecord(
  result: ReturnType<typeof createSaltUi>,
...
function hasImplementationStarterCode(
  snippets: StarterCodeSnippet[] | undefined,
): boolean {
  return Boolean(
    snippets?.some((snippet) => snippet.label !== "Grounded Salt icon slots"),
  );
}
...
export function buildTranslationRecords(
  ...
  const recommendationCache = new Map<
    string,
    ReturnType<typeof createSaltUi>
  >();
  ...
  return sourceModel.ui_regions.map((source) => {
    ...
    const effectiveArchetype =
      source.kind === "navigation" && contextualSource.scope === "app-structure"
        ? getArchetype("vertical-navigation")
        : archetype;
    ...
    const effectiveIntentProfile = {
      ...
      preferred_categories: (() => {
        const refinedCategories =
          effectiveArchetype.solution_type === "pattern" &&
          (contextualHints.length > 0 || contextualQueryHints.length > 0)
            ? refinePreferredCategoriesForSource(
                semanticIndex,
                contextualSource,
                effectiveArchetype.solution_type,
    ...
    const cacheKey = createDecisionCacheKey({
      ...
      solutionType: effectiveArchetype.solution_type,
      ...
      formFieldSupport:
        effectiveArchetype.require_form_field_support ??
        input.form_field_support,
    ...
    const recommendation =
      ...
      (() => {
        const result = createSaltUi(registry, {
          query: effectiveIntentProfile.query,
          solution_type: effectiveArchetype.solution_type,
    ...
    const migrationKind = getMigrationKind(
      effectiveArchetype.migration_kind,
      hasAcceptableTarget,
      confidence,
    );
    ...
    return {
      ...
      label: effectiveArchetype.label,
      ...
      implementation: {
        ...
        starter_code_available: hasImplementationStarterCode(starterCode),
      },
      salt_target: {
