...
export function resolveGuideIdentifierLookup(
  guides: GuideRecord[],
  query: string,
): GuideLookupResult {
  ...
  const slugMatches = guides.filter((guide) => {
    const overviewSlug = getGuideOverviewSlug(guide);
    return overviewSlug
      ? normalizeGuideLookup(overviewSlug) === normalizedName
      : false;
  });
...
export function resolveGuideLookup(
  ...
  const normalizedName = normalizeGuideLookup(query);
  const resolved = resolveGuideIdentifierLookup(guides, query);
