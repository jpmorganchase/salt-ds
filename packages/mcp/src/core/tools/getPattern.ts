...
import { buildPatternPresentationBase } from "./solutionPresentation.js";
...
const MAX_COMPACT_PATTERN_EXAMPLES = 3;
...
function toCompactPattern(
  ...
  if (include.includes("examples")) {
    compact.examples = pattern.examples.slice(0, MAX_COMPACT_PATTERN_EXAMPLES);
    compact.example_count = pattern.examples.length;
    compact.examples_truncated =
      pattern.examples.length > MAX_COMPACT_PATTERN_EXAMPLES;
  }
...
export function getPattern(
  ...
  return {
    pattern: projectLookupRecord(resolution.candidate, input.view, {
      toCompact: (pattern) =>
        toCompactPattern(registry, pattern, input.include),
    }),
