...
function getMissingProviderImportWarning(provider: string): string {
  return `Theme defaults declare custom provider ${provider} without provider_import metadata. Repo policy can describe that provider, but implementation guidance cannot provide its exact import until provider_import is supplied.`;
}
