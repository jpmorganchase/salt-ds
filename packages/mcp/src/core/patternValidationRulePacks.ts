  ...
  orderedRegions?.values.forEach((value, index) => {
    ...
    rules.push(
      buildRule({
        ...
        claim_kind: "pattern",
        unsupported_reason:
          "Salt records source-backed starter regions but does not yet validate rendered region order or layout behavior.",
      }),
    );
  });
    ...
    (value, index) => {
      ...
      rules.push(
        buildRule({
          ...
          claim_kind: "pattern",
          unsupported_reason:
            "Salt records source-backed preserve constraints but does not yet validate layout or interaction behavior.",
        }),
      );
    },
  );
  ...
  pattern.accessibility.summary.forEach((value, index) => {
    ...
    rules.push(
      buildRule({
        ...
        unsupported_reason:
          "Salt records source-backed pattern accessibility guidance but does not yet validate pattern-level accessibility behavior.",
      }),
    );
  });
...
const REGISTRY_PATTERN_RULE_PACK_CACHE = new WeakMap<
  SaltRegistry,
  SaltPatternValidationRulePack
>();

/** Derive the rule pack from the same in-memory patterns used by review. */
export function getRegistryPatternValidationRulePack(
  registry: SaltRegistry,
): SaltPatternValidationRulePack {
  const cached = REGISTRY_PATTERN_RULE_PACK_CACHE.get(registry);
  if (cached) {
    return cached;
  }
  ...
  const rulePack = buildPatternValidationRulePack({
    ...
    generator: {
      name: "salt-mcp.pattern-validation",
    },
  });
  REGISTRY_PATTERN_RULE_PACK_CACHE.set(registry, rulePack);
  return rulePack;
}
