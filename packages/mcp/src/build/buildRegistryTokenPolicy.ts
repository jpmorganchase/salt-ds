...
const TOKEN_POLICY_DOC = "/salt/themes/design-tokens/token-usage-rules";
...
function buildPalettePolicy(): NonNullable<TokenRecord["policy"]> {
  return {
    ...
    avoid_for: ["direct component styling", "pattern styling", "custom UI color selection"],
    notes: [
    ...
    docs: withDocs(TOKEN_POLICY_DOC, DESIGN_TOKENS_DOC),
  };
}
...
function buildCharacteristicPolicy(
  ...
  const characteristicDoc =
    CHARACTERISTIC_DOC_BY_CATEGORY[category] ?? "/salt/themes/design-tokens/how-to-read-tokens";
  ...
  if (category === "content") {
    preferredFor.push("text foreground", "icon foreground", "default static foreground");
    notes.unshift(
    ...
    preferredFor.push("navigation states", "navigation indicators", "navigation emphasis");
    ...
    preferredFor.push("error states", "warning states", "success states", "informational states");
  ...
  return {
    ...
    avoid_for: avoidFor,
    notes,
    docs: withDocs(TOKEN_POLICY_DOC, DESIGN_TOKENS_DOC, characteristicDoc),
  };
}
...
function buildFoundationPolicy(
  ...
  const foundationDoc = FOUNDATION_DOC_BY_CATEGORY[category] ?? FOUNDATIONS_INDEX_DOC;
  const preferredFor = ["low-level structural styling"];
  const avoidFor = ["semantic component styling when a characteristic token fits"];
  const notes = [
  ...
  if (category === "size") {
    ...
    if (tokenName.includes("--salt-size-fixed-")) {
      preferredFor.push("border thickness", "separator thickness", "constant dimensions across densities");
      notes.unshift(
    ...
    preferredFor.push("low-level color infrastructure", "cases without a semantic characteristic equivalent");
    ...
    preferredFor.push("type scale", "font family", "font weight", "text metrics");
  ...
  return {
    ...
    avoid_for: avoidFor,
    notes,
    docs: withDocs(TOKEN_POLICY_DOC, DESIGN_TOKENS_DOC, foundationDoc),
  };
}
...
export function getTokenPolicy(
  ...
  if (Object.hasOwn(CHARACTERISTIC_DOC_BY_CATEGORY, category)) {
    return buildCharacteristicPolicy(category);
  }
