...
function buildGroupings(
  ...
  const regionIdsByKind = new Map(pageRegions.map((region) => [region.kind, region.id]));
  const uiIdsByKind = new Map(uiRegions.map((region) => [region.kind, region.id]));

  if (
    regionIdsByKind.has("sidebar") &&
  ...
  if (
    ...
    pushGrouping(groupings, {
      ...
      ui_region_refs: [
        ...(uiIdsByKind.has("dialog") ? [uiIdsByKind.get("dialog") as string] : []),
      ],
      reason:
  ...
  if (
    ...
    pushGrouping(groupings, {
      kind: "form-flow",
      label: "Form flow",
      region_refs: [regionIdsByKind.get("form-section") as string],
      ui_region_refs: [
      ...
      reason:
        "The source includes grouped fields and controls that should be translated as a form section rather than isolated primitives.",
    });
  }
  ...
  if (
    ...
    pushGrouping(groupings, {
      ...
      ui_region_refs: [uiIdsByKind.get("data-table") as string],
      reason:
...
export function buildSourceUiModel(
  ...
  return {
    ...
    groupings,
    summary: {
      dominant_scope: summarizeDominant(uiRegions.map((region) => region.scope)),
