...
beforeAll(async () => {
  [catalogDirectory, projectRoot] = await Promise.all([
    createBuiltCatalogV2Fixture("salt-phase2-outcomes-"),
    fs.mkdtemp(path.join(os.tmpdir(), "salt-phase2-project-")),
  ]);
...
describe("Phase 2 negative outcomes", () => {
