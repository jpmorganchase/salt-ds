...
describe("sourceAttribution", () => {
  ...
  it("uses workflow provenance as the source of truth for workflow envelopes", () => {
    const sources = collectToolSources({
      workflow: {
        id: "create_salt_ui",
        provenance: {
      ...
      result: {
        raw: {
          href: "https://example.com/should-not-be-discovered",
        },
      },
    });
    ...
    expect(sources).toEqual([
      {
        original: "/salt/patterns/metric",
        resolved: "https://www.saltdesignsystem.com/salt/patterns/metric",
        kind: "site",
      },
      {
        original: "packages/core/src/button.tsx",
        resolved: "packages/core/src/button.tsx",
        kind: "repo",
      },
    ]);
  });
