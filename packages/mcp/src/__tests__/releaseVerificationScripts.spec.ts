...
const POST_BUILD_STEPS = [
  ...
  "yarn check:ai-tooling:pack -- --report dist/salt-ai-pack/release-complete.json",
  "yarn smoke:consumer --skip-build --pack-report dist/salt-ai-pack/release-complete.json",
];
...
describe("release verification scripts", () => {
  ...
  it("runs all package gates after the PR build", async () => {
    ...
    const mcpPackageGate = workflow.indexOf(
      "run: yarn check:ai-tooling:pack -- --report dist/salt-ai-pack/release-complete.json",
    );
