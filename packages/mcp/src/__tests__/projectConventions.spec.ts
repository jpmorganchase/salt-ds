...
describe("project conventions example contract", () => {
  it.each([
    ...
    const schema = readJson(
      "../../../semantic-core/schemas/project-conventions.schema.json",
    );
  ...
  it("validates the consumer stack manifest against the stack schema", () => {
    ...
    const schema = readJson(
      "../../../semantic-core/schemas/project-conventions-stack.schema.json",
    );
...
describe("mergeCanonicalAndProjectConventionLayers with one repo layer", () => {
  ...
  it("carries declared theme and token policy overlays without rewriting the canonical decision", () => {
    const merged = mergeWithSingleRepoLayer(
      {
        decision: {
          name: "Button",
        ...
        guidance_boundary: {
          ...
          project_conventions: {
            ...
            contract: "project_conventions_v1",
            check_recommended: true,
            topics: ["wrappers", "tokens", "theme"],
            reason: "Check repo policy before finalizing the implementation.",
          },
        },
      },
      {
        token_aliases: [
          {
            salt_name: "--salt-container-primary-borderColor",
            prefer: "--repo-border-default",
            reason: "Container border roles map through the repo token alias.",
          },
        ],
        theme_defaults: {
          provider: "RepoThemeProvider",
          provider_import: {
            from: "@/theme/RepoThemeProvider",
            name: "RepoThemeProvider",
          },
          imports: ["@/theme/repo-theme.css"],
          props: [{ name: "density", value: "high" }],
          reason: "The repo applies one approved theme provider around Salt.",
        },
        token_family_policies: [
          {
            family: "container",
            mode: "prefer-local-aliases",
            reason: "Container roles should use local aliases when available.",
          },
        ],
      },
    );
      ...
      },
      {
        ...
        conventions: {
          ...
          token_aliases: [
            {
              salt_name: "--salt-container-primary-borderColor",
              prefer: "--team-border-default",
              reason: "Team aliases container tokens locally.",
            },
          ],
          theme_defaults: {
            provider: "TeamThemeProvider",
            provider_import: {
              from: "@/theme/TeamThemeProvider",
              name: "TeamThemeProvider",
            },
            reason: "Team theme shell.",
          },
          token_family_policies: [
            {
              family: "container",
              mode: "prefer-local-aliases",
              reason: "Prefer team aliases for container tokens.",
            },
          ],
