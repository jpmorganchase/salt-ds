...
import { compareOptions } from "../core/tools/compareOptions.js";
import { createSaltUi } from "../core/tools/createSaltUi.js";
import { getComponent } from "../core/tools/getComponent.js";
import { getCompositionRecipe } from "../core/tools/getCompositionRecipe.js";
import {
  buildPatternRecipe,
  toRecipeStarterCode,
} from "../core/tools/getCompositionRecipeHelpers.js";
import { getCountrySymbol } from "../core/tools/getCountrySymbol.js";
import { getCountrySymbols } from "../core/tools/getCountrySymbols.js";
import { getFoundation } from "../core/tools/getFoundation.js";
import { getGuide } from "../core/tools/getGuide.js";
import { getIcon } from "../core/tools/getIcon.js";
import { getIcons } from "../core/tools/getIcons.js";
import { getPage } from "../core/tools/getPage.js";
import { getPattern } from "../core/tools/getPattern.js";
import { getRelatedEntities } from "../core/tools/getRelatedEntities.js";
import { getSaltEntities } from "../core/tools/getSaltEntities.js";
import { getSaltEntity } from "../core/tools/getSaltEntity.js";
import { getToken } from "../core/tools/getToken.js";
import { listFoundations } from "../core/tools/listFoundations.js";
...
describe("getComponent", () => {
  it("returns compact component data with optional sections", () => {
    const result = getComponent(REGISTRY, {
      name: "Button",
      include: ["props", "accessibility", "deprecations"],
    });
    ...
    expect(result.suggested_follow_ups).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          args: expect.objectContaining({
            query: "Compare Button with Link",
  ...
  it("caps component examples and deduplicates shared examples across a batch", () => {
    const buildExample = (id: string, code: string) => ({
      id,
      title: id,
      ...
      code,
      source_url: `/salt/examples/${id}`,
      package: "@salt-ds/core",
      target_type: "component" as const,
      target_name: "Fixture",
    });
    const shared = buildExample("shared", "<SharedExample />");
    const examplesByComponent = new Map([
      [
        "Button",
        [
          shared,
          buildExample("button-1", "<Button>One</Button>"),
          buildExample("button-2", "<Button>Two</Button>"),
          buildExample("button-3", "<Button>Three</Button>"),
          buildExample("button-4", "<Button>Four</Button>"),
        ],
      ],
      [
        "Link",
        [shared, buildExample("link-1", '<Link href="/next">Next</Link>')],
      ],
      [
        "VerticalNavigation",
        [
          buildExample("navigation-1", "<VerticalNavigation />"),
          buildExample(
            "navigation-2",
            '<VerticalNavigation aria-label="Two" />',
          ),
          buildExample(
            "navigation-3",
            '<VerticalNavigation aria-label="Three" />',
          ),
        ],
      ],
    ]);
    const registry: SaltRegistry = {
      ...
      components: REGISTRY.components.map((component) => ({
        ...component,
        examples: examplesByComponent.get(component.name) ?? component.examples,
      })),
    };

    const button = getComponent(registry, {
      name: "Button",
      include: ["examples"],
    }).component as {
      examples: unknown[];
    ...
    expect(button.example_count).toBe(5);
    expect(button.examples_truncated).toBe(true);

    const batch = getSaltEntities(registry, {
      names: ["Button", "Link", "VerticalNavigation"],
      include: ["examples"],
    });
    const entities = batch.results.map(
      (row) => row.result.entity as Record<string, unknown>,
    );
    const includedExamples = entities.flatMap((entity) =>
      Array.isArray(entity.examples) ? entity.examples : [],
    ) as Array<{ code?: string }>;
    const includedCodes = includedExamples.map((example) => example.code);

    expect(includedExamples).toHaveLength(3);
    expect(includedCodes.filter((code) => code === shared.code)).toHaveLength(
      1,
    );
    expect(
      entities.some(
        (entity) =>
          typeof entity.examples_omitted_in_batch === "number" &&
          entity.examples_omitted_in_batch > 0,
      ),
    ).toBe(true);
  });

  it("omits oversized example code while preserving source metadata", () => {
    const sourceExample = REGISTRY.components.find(
      (component) => component.name === "Button",
    )?.examples[0];
    if (!sourceExample) {
      throw new Error("Button fixture must include a canonical example.");
    }
    const oversized = {
      ...sourceExample,
      id: "button.oversized",
      title: "Oversized Button example",
      source_url: "/salt/components/button/examples",
      code: "x".repeat(30_000),
    };
    const registry: SaltRegistry = {
      ...REGISTRY,
      components: REGISTRY.components.map((component) =>
        component.name === "Button"
          ? { ...component, examples: [oversized] }
          : component,
      ),
    };
    ...
    const entity = batch.results[0]?.result.entity as
      | Record<string, unknown>
      | undefined;
    const example = Array.isArray(entity?.examples)
      ? (entity.examples[0] as Record<string, unknown>)
      : null;
    ...
    expect(example).toMatchObject({
      ...
      source_url: "/salt/components/button/examples",
      code_bytes: 30_000,
      code_omitted_in_batch: true,
    });
    expect(example).not.toHaveProperty("code");
    expect(entity?.example_codes_omitted_in_batch).toBe(1);
    expect(Buffer.byteLength(JSON.stringify(batch), "utf8")).toBeLessThan(
      20_000,
    );
  });
    ...
    expect(result.suggested_follow_ups).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          workflow: "create_salt_ui",
          args: expect.objectContaining({
            query: expect.stringContaining("Compare Split button with"),
...
describe("consumer tools", () => {
  ...
  it("can enforce production-ready filters on component recommendations", () => {
    ...
    expect(result.next_step).toBe(
      "Broaden the task description, or inspect a likely exact component name with get_salt_reference.",
    );
  });
  ...
  it("does not invent generic starter code for a pattern without a canonical scaffold", () => {
    const result = getCompositionRecipe(REGISTRY, {
      query: "primary action with alternatives",
      top_k: 2,
      include_starter_code: true,
    ...
    expect(result.recommended?.name).toBe("Split button");
    expect(result.starter_code).toEqual([]);
  });
          ...
          starter_scaffold: {
            ...
            template: {
              ...
              imports: [
                ...
                { name: "Tabs", package: "@salt-ds/core" },
              ],
              jsx_lines: [
                "<BorderLayout>",
  ...
  it("withholds a draft pattern scaffold instead of inventing generic composition", () => {
    const registry: SaltRegistry = {
      ...REGISTRY,
      patterns: [
        ...REGISTRY.patterns,
  ...
  it("preserves the app header shell when starter code is requested", () => {
          ...
          starter_scaffold: {
            ...
            template: {
              ...
              jsx_lines: [
                ...
                '      <Button appearance="bordered">Settings</Button>',
                "    </StackLayout>",
                "  </StackLayout>",
                "</header>",
              ],
    ...
    expect(starterCode?.[0]?.code).not.toContain('appearance="secondary"');
    expect(starterCode?.[0]?.notes).toContain(
      "Keep the header shell recognizable instead of reducing it to isolated navigation and action components.",
    );
  });
  ...
  it("translates external or generic UI descriptions into Salt targets", () => {
    ...
    expect(result.implementation_plan.starter_code_targets ?? []).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          source_kind: "split-action",
          target_name: "Split button",
    ...
    expect(result.starter_code ?? []).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: "Split button starter" }),
      ]),
    );
  ...
  it("keeps migration rerun follow-up arguments on the public MCP surface", () => {
    const result = migrateToSalt(REGISTRY, {
      query: "Replace a primary action with Salt",
      include_starter_code: false,
    });
    const rerun = result.suggested_follow_ups?.find(
      (followUp) => followUp.workflow === "migrate_to_salt",
    );
...
describe("curated public tools", () => {
  ...
  it("preserves normalized aliases without fuzzily substituting unknown names", () => {
    const aliasResult = getSaltEntity(REGISTRY, {
      entity_type: "pattern",
      name: "  SPLIT action  ",
    });
    const unknownResult = getSaltEntity(REGISTRY, {
      name: "Butto",
    });

    expect(aliasResult.decision.status).toBe("found");
    expect(aliasResult.entity).toMatchObject({ name: "Split button" });
    expect(unknownResult.decision.status).toBe("not_found");
    expect(unknownResult.entity).toBeNull();
    expect(unknownResult.matches).toBeUndefined();
  });
});
