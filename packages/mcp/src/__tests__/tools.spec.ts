...
import { mergeCanonicalAndProjectConventionLayers } from "../../../project-conventions-runtime/src/index.js";
import {
  compareOptions,
  compareVersions,
  getChanges,
...
import { isComponentAllowedByDocsPolicy } from "../../../semantic-core/src/tools/utils.js";
import {
  chooseSaltSolution,
  compareSaltVersions,
  discoverSalt,
...
describe("consumer tools", () => {
  ...
  it("includes routing provenance in full discover_salt output", () => {
    ...
    expect(result.raw).toMatchObject({
      routing_debug: {
        chosen_workflow: "choose_salt_solution",
...
describe("curated public tools", () => {
  ...
  it("includes decision provenance in full choose_salt_solution output", () => {
    const registry: SaltRegistry = {
      ...REGISTRY,
      components: REGISTRY.components.map((component) =>
        component.name === "Link"
  ...
  it("can merge a real choose_salt_solution result with project conventions through the runtime package", () => {
    const canonical = chooseSaltSolution(REGISTRY, {
      query: "navigate to another route",
    });
