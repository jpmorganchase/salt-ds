...
import { reviewSaltUi } from "@salt-ds/semantic-core";
 ...
 * canonical Salt reference: it is what `get_salt_examples` should return
 * and what consumer code should look like after Salt's create / migrate /
 * upgrade workflows. If `review_salt_ui` raises a blocking finding against
 ...
 *
 * Per the roadmap, this spec is allowed to fail today: its failure output
 * IS the "Heuristics to repair" list that the follow-up PR closes. Do not
 * fix flagged heuristics here, and do not modify pattern stories — stories
 * are the canonical reference; heuristics flex around them.
 *
 * "Blocking" here = ValidationIssue.severity === "error", matching the
 * severity tier semantic-core reserves for findings that should block
...
describe("canonical-example review round-trip (roadmap task 0.8 / F4)", () => {
  ...
  it("review_salt_ui raises zero blocking findings for every canonical pattern story", async () => {
    ...
    expect(
      ...
      } file(s) of ${stories.length} story file(s) total).\n\nHeuristics to repair (the spec FAILS today on purpose — this list IS the input for the follow-up heuristic fixes; do not edit pattern stories to silence them):\n${formatFindings(
        blocking,
      )}`,
    ).toEqual([]);
  }, 120_000);
