...
export function buildSaltMcpInstructions(registry: SaltRegistry): string {
  ...
  return [
    ...
    "When the top-level workflow contract reports status = partial or blocked, do not treat starter-code generation or early file creation as completion. Follow the returned action or explicitly report that the workflow is still incomplete.",
    "Request view = full only after compact output is insufficient or the user explicitly asks for starter code, richer provenance, or detailed implementation artifacts. Do not request full output just to fix context or guess past blocked compact output.",
    "Leave solution_type unset for broad or mixed-surface create prompts unless the request already points clearly to a known Salt family. Forcing solution_type = pattern on broad create asks can reduce first-pass routing quality.",
