...
export function buildSaltMcpInstructions(registry: SaltRegistry): string {
  ...
  return [
    ...
    "Workflow follow-ups use stable public tool IDs. The top-level action is the single authoritative next step; after a deterministic create reference retrieval succeeds, run its one-level action.post_action when present. Dependency installation stays outside the semantic workflow contract. ask_user stops with action.post_action set to null and requires updated user input.",
