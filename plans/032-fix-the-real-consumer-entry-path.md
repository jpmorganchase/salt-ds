...
2. Add negative tests for the current retrieval Core/Lab and Core/Theme disagreement. Rebind affected new fixture identities without rewriting terminal historical evidence.
   ...
   The preserved 13-case `baseline-pre-platform.json` belongs to Plan 004/01 at `da1d249225c7044dad1c3aa08c960eb08f84dddb`, recorded with its report/manifest/fixture paths in the unchanged Plan 004 index. Its bytes and recorded identities match that snapshot. The original 14-case Plan 001/00b content-addressed report is separate evidence. Preserve both; do not restore one over the other. Current CI needs the recorded Git history to verify those frozen inputs.
...
Scope: `packages/cli/src/cli.ts`, `packages/cli/src/commands/{info,docs,context,retrievalRuntime}.ts`, relevant Knowledge project-facts/package/workspace metadata readers, focused selection tests, and packed consumer fixtures. Update `packages/cli/README.md` and the selection contract in `docs/ai/knowledge-bundle.md` with the implemented flags and path semantics. These documentation changes keep the current installation/retrieval instructions aligned with the new command interface. Reuse the existing bounded metadata/containment helpers. Full source discovery, analyzer rules and scanner configuration are not prerequisites for core guidance.

1. Define one explicit selection shape containing repository authority and the application workspace within it. Add `--root <repo>` and `--project <relative-workspace>` to `info`, `docs`, and `context`. Omitted `--root` means the current working directory; omitted `--project` means `.` within that authority. Never select a child automatically. Replace the old positional `info` root argument and update all current callers; the unreleased interface needs no alias. Report project and evidence paths relative to the canonical repository authority, so a selected child's hoisted dependencies remain representable without absolute paths. Doctor retains its existing aggregate behavior only until its separate retirement; do not use it to obtain core project selection.
