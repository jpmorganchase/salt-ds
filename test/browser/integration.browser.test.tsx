...
function AnnounceButton() {
  ...
  return (
    <button type="button" onClick={() => announce("Integration announcement")}>
      Announce
    </button>
  );
}

describe("Vitest Browser Mode integration", () => {
  ...
  it("retries assertions for ARIA live-region updates", async () => {
    await renderWithSalt(
      <AriaAnnouncerProvider data-testid="integration-announcer">
        <AnnounceButton />
    ...
    await expect
      .element(page.getByTestId("integration-announcer"))
      .toHaveTextContent("Integration announcement");
  });
});
