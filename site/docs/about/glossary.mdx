...
For disabled components, status information is not displayed. This approach is based on accessibility principles, as disabled elements don't require visibility in the UI.

In contrast, read-only components display status information. These elements contain relevant information applicable to the workflow. Displaying status ensures accessibility and communicates necessary information.
