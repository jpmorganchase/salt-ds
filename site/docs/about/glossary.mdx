...
For disabled components, status icons are not displayed. This approach is based on accessibility principles, as disabled elements don't require visibility in the UI.

In contrast, read-only components display status icons. These elements contain relevant information applicable to the workflow. Displaying status icons ensures accessibility and communicates necessary information.
