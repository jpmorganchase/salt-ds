...
A component in a disabled state is not considered relevant to the user’s workflow, as the user doesn’t need to know the component is there. It doesn't need to meet minimum contrast requirements, and a progressive reveal might be a better UI pattern. While buttons can use the focusableWhenDisabled prop, the system doesn't support status messages for disabled components. If a component is relevant, consider using a read-only state instead.

## Read-only state 

A component in a read-only state is considered relevant to the user’s workflow, as users need to know it’s there. It must meet accessibility requirements, including minimum contrast. The system supports status messages for read-only components, as RAG states may require visibility for recognition and further action. If not relevant, a disabled state may be more appropriate.
