...
Salt Design System provides building blocks for accessible digital experiences. We target Web Content Accessibility Guidelines (WCAG) 2.2 Level AA and incrementally record evidence for individual components. A target is not the same as tested behavior, supported assistive technology or formal conformance. See the [accessibility evidence model](/salt/about/accessibility-evidence) for definitions, current evidence and limitations.
...
JPM Brand theme is Salt’s long-term visual style, designed to match the JPMorgan brand across CIB digital applications. Developed with Corporate Brand and CIB Marketing, it uses approved styles for consistent visuals on desktop and web. See all the characteristics for [JPM Brand theme](/salt/themes/design-tokens/tokens).

### Legacy (UITK) theme

Legacy (UITK) theme emulates Salt's predecessor, UI Toolkit, and supports migration from the older design system. See all the characteristics for [Legacy theme](/salt/themes/design-tokens/tokens).

### Chase theme

The Chase theme brings the Chase consumer brand’s look and feel to Salt. It's available exclusively to JPMorganChase employees. For more details, see the [Chase theme documentation](https://go/saltds-chase-theme) (internal only).
