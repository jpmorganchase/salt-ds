...
## Customizing with Salt Next theme
...
Salt Next theme brings the [JPM brand](https://jpmcbrandtoolbox.com/) identity to CIB (Commercial & Investment Bank) digital applications.
...
Salt Next theme introduces the Amplitude typeface, taken directly from the CIB Brand Guidelines, for use in header and display sizes as well as button labels. Open Sans remains the primary typeface for body copy text sizes and smaller in the Salt Next type ramps.
...
'Legacy' theme does not provide any styling options other than density and mode. We do not recommend customizing the look-and-feel of the 'Legacy' theme. Please be aware that you are responsible for maintaining any customizations you make to this theme.
...
Using Figma variables enables us to reduce the number of Figma libraries we need to provide. Salt Next theme has the following libraries:

1. Salt (Next) Components & Patterns
2. Salt (Next) Style Library
3. Salt (Next) Color Library

The 'Salt Icons' library has been updated with variable support to work with Salt Next theme.
