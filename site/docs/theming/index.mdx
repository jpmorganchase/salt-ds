...
[Image: Buttons at different densities]

##### Mode

Switches between a light and dark color palette.

[Image: LD Buttons in light or dark]

##### Accent

Determines the dominant color: blue (aligned to the legacy design language) or teal (aligned to the JPM MasterBrand Styleguide).

[Image: LD Buttons in blue or teal]

##### Corner radius

Sets predefined corners in certain components to sharp or round.

[Image: LD Buttons in round or sharp]

##### Font

Displays heading and/or action fonts as Amplitude or Open Sans.

[Image: LD Buttons in Amplitude or Open Sans]
...
[Image: Representative composition ]
...
**Example #3**: An internal application is frequently to handle operational tasks. An LOB define their design language to be medium density, light mode, round corners, open sans font, teal accent.
...
Any theme changes outside of 'style options' mean you assume certain responsibilities from Salt. For example, if you change a token for a background color, you must ensure it meets accessibility standards for contrast ratios.
...
### Component-level style options

In addition to the Salt Next theme, Salt is beginning to provide further component-level style options for full borders (where the border is considered decorative) and drop shadows. The options are on a component-by-component basis and not a system-wide style option. You can choose whether to have these options on each part of the application.
...
[Image: LD Form Field with just underline, sharp, blue vs border, round, teal]

## JPM Brand endorsed

Salt Next brings the JPM brand identity to CIB (Commercial & Investment Bank) digital applications.
...
The Salt Next theme is now stable and available in both Figma and code. In Figma, options are changed through variable mode switches at a Page, Frame or Component level. In code, they are provided as props to `SaltProviderNext`.

### Figma

#### New variable-driven libraries

To provide ‘style options’ and theming capability the new Salt Next Figma libraries implement Salt design tokens as Figma variables to define all the visual attributes of components.
...
The Style Library includes all the Figma variables needed to design for Salt. It includes the semantic palette, category colors, size and spacing values, corner radiuses, typographic styles and more, The variables are set up to match our tokens in code. When inspecting a design in Figma's Dev Mode, you can see references that match our tokens, such as size and spacing values (e.g., `-salt-spacing-300`).
...
Note: You don't need to add Foundation Colors to your project unless you want to use them directly. For the best experience, apply styles from the Salt Next Style library to your project only.

#### Using the Component & Pattern Library

The new component library is built using the new style variables. It functions similarly to the current Salt component library but with key differences:

- No need to switch between light and dark mode libraries.
- No need to set 'density' at the component level.
- Access new features like rounded corners, teal as an accent color, and Amplitude as a heading and/or action font.

#### How to Change the Style of a Component

1. Drag a Component: From the Assets panel in the right sidebar, drag a component onto a page.
2. Access Variable Options: In the left sidebar, click on the Variables icon in the Layer panel to see all available variable options.
3. Change the Component:
4. Accent / Mode: Change the accent color (e.g., indicators in Tabs, CTA buttons) and/or the mode (light/dark).
5. Corner: Switch between sharp edges (default) and rounded corners (note: not all components have rounded corners).
6. Density: Change the size and spacing of elements, as well as corner radius and typographic values (font size and line height). The default is HD.
7. Heading Font: Change the font family of Display and H1-4 styles from Open Sans to Amplitude.
8. Action Font: Change the font family of action styles from Open Sans to Amplitude.

By following these steps, you can fully utilize the new style variables and features in the Salt Next libraries.

#### Theme Recipes and Ingredients

In Salt, a theme is made up of a series of choices, known as a 'theme recipe.' The available choices are:

- Density: High, Medium, Low, Touch
- Mode: Light or Dark
- Accent Color: Blue or Teal
- Corner: Sharp or Round
- Heading Font: Open Sans or Amplitude
- Action Font: Open Sans or Amplitude

In code, these choices can be combined into a 'preset' theme for consistent use across a LOB's products.

#### Setting a Theme Recipe in Figma

Variable options can be set at different levels within Figma. While you can configure them at an individual component level, a theme should ideally be defined at a product or LOB level. This ensures consistency and avoids setting variables component by component.
