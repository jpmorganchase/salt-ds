...
Salt uses tokens, which store design decisions in a single place. Components can use these tokens to set their style.

<ExampleContainer type="positive" customPillText="Best practice">
  When theming, we recommend modifying tokens rather than tweaking designs for each individual component.
</ExampleContainer>
