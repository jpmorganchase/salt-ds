...
Content status is a pattern that provides information when content isn’t ready for display in a container component of an application.
...
We provide design guidance for using [`Button`](/salt/components/button), [`Progress`](/salt/components/progress), [`Spinner`](/salt/components/spinner), [`StackLayout`](/salt/components/stack-layout), [`StatusIndicator`](/salt/components/status-indicator), and [`Text`](/salt/components/text) to achieve the pattern. You can also find implementation examples in the [Storybook](https://storybook.saltdesignsystem.com/?path=/story/patterns-content-status--info).
