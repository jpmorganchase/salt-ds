...
- Use [`StackLayout`](/salt/components/layout/stack-layout) to display the two lists and move buttons that make up list builder in a row formation.
- Use [`Text`](/salt/components/text), [`Button`](/salt/components/button) and [`Input`](/salt/components/input) to assemble the header area.
- Use [`StackLayout`](/salt/components/layout/stack-layout) to orient the move buttons between the two lists in a column formation.
