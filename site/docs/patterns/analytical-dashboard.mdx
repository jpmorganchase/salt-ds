...
- [`BorderLayout`](/salt/components/layout/border-layout): Use the border layout component to manage the top-level layout of the dashboard, including header, footer and navigation panel elements.
- [`FlowLayout`](/salt/components/layout/flow-layout): Use for small-scale layouts that require components (such as cards) to be displayed in a responsive row.
- [`StackLayout`](/salt/components/layout/stack-layout): Use for small-scale layouts that require displaying components in a single row or column and may benefit from the supported visual separators.
...
- [`GridLayout`](/salt/components/layout/grid-layout): Use to manage the layout and responsiveness of the main content area in your dashboard.
...
- Layout is flexible depending on the dashboard grid. A metrics bar can span the full width of a dashboard (e.g., over 12 columns) or follow a [`SplitLayout`](/salt/components/layout/split-layout) using the remaining number of available columns across the main body of the dashboard. Use the [`StackLayout`](/salt/components/layout/stack-layout) or [`FlowLayout`](/salt/components/layout/flow-layout) components to organize the layout within a metric bar depending on whether metrics should wrap or scroll within a module.
