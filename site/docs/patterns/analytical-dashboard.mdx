...
- When the content is mostly a single simple metric, use [Metric](/salt/patterns/metric).
- When the content is primarily a simple table or interactive grid, use [Table](/salt/components/table) or [Data grid](/salt/components/ag-grid-theme).

## How to build

Follow [Content status guidance for loading and recovery](/salt/patterns/content-status#choosing-how-to-communicate-loading-and-recovery) when dashboard regions load or refresh, including feedback when previously loaded data remains visible after a failed refresh.
...
1. **Dashboard header**: The top region introduces the context of the dashboard and can contain branding, status, or summary actions.
2. **Key metrics**: A compact strip of metric cards or metrics highlights the most important values first.
3. **Fixed controls or filters**: Persistent controls sit outside the main body so the user can refine the data without losing context.
4. **Main analytical body**: The main content area contains charts, tables, or other widgets that provide the detailed analysis.

Other features typically found in an analytical dashboard include:

- **Data grid or table**: Table supports application-filtered rows and cell actions. Use Data grid for integrated grid interactions.
...
- [`BorderLayout`](/salt/components/layouts/border-layout): Use the border layout component to manage the top-level layout of the dashboard, including header, footer and navigation panel elements.
- [`FlowLayout`](/salt/components/layouts/flow-layout): Use for small-scale layouts that require components (such as cards) to be displayed in a responsive row.
- [`StackLayout`](/salt/components/layouts/stack-layout): Use for small-scale layouts that require displaying components in a single row or column and may benefit from the supported visual separators.
...
- [`GridLayout`](/salt/components/layouts/grid-layout): Use to manage the layout and responsiveness of the main content area in your dashboard.
...
1. Dashboard header
2. Key metrics
3. Fixed controls or filters
4. Charts, graphs, tables, or data grids in the main analytical body

<GuidanceCallout type="positive" customPillText="Best practices">

- Use [`Drawer`](/salt/components/drawer) for temporary overlays that do not own the dashboard layout.
- Keep persistently displayed panels in the west region of the border layout so they stay visible without hiding the main analytical body.
- Use `BorderLayout` for the top-level shell and `GridLayout` for the main analytical body.
- Keep metrics visually compact and keep the main analytical body focused on analysis, not navigation.
...
- Layout is flexible depending on the dashboard grid. A metrics bar can span the full width of a dashboard (e.g., over 12 columns) or follow a [`SplitLayout`](/salt/components/layouts/split-layout) using the remaining number of available columns across the main body of the dashboard. Use the [`StackLayout`](/salt/components/layouts/stack-layout) or [`FlowLayout`](/salt/components/layouts/flow-layout) components to organize the layout within a metric bar depending on whether metrics should wrap or scroll within a module.
