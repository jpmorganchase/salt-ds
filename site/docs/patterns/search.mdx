...
<Callout title="Search results data limitation">
Please note that the current Combo Box has a limit on the number of search results it can handle efficiently. 
For large data sets, consider methods like limiting the results or directing users to a separate search results page to enhance the user experience. 
We are actively working on improving the Combo Box for large data sets. 
Check this [GitHub ticket](https://github.com/jpmorganchase/salt-ds/issues/4079) to track progress.
</Callout>
