...
<LivePreview componentName="patterns/search" exampleName="DefaultIcon" />
...
- When users already know the destination and only need a direct navigation control, use a link, button, or navigation item instead.
- When the task is filtering a visible data set in place, use filters or a data grid instead of Search.
- When the scope is a small, fixed set of options, use a Combo box or Select control instead of Search.

## How to build

### Choosing and submitting a result

Choosing a suggestion and submitting a free-text query are separate actions:

- When the suggestion list is open and an enabled suggestion is active, **Enter** chooses that active suggestion. It does not always choose the first result. Users can change the active suggestion with the arrow keys or choose a result directly.
- If free-text search is supported and no suggestion is active, **Enter** can submit the typed query to a results page. A dedicated **Search** button can also explicitly submit that query.
- The host application owns navigation and query submission. Selecting a suggestion must not also submit the text query or trigger a second navigation.

Use [`ComboBox`](/salt/components/combo-box) selection callbacks for choosing a suggestion, and keep query submission separate. Follow [List filtering](/salt/patterns/list-filtering#focus-and-result-updates) for focus, asynchronous updates, and result feedback.

### Anatomy

A search pattern can be present in an application as a global or focused search, depending on the scope of the search functionality. Independently of how a search is presented, the following elements are present in a search pattern:
...
4. **Close button:** A close button that appears within the search input field, allowing users to quickly clear their current query and start a new search.
...
- Use [`Combo box`](/salt/components/combo-box) to display the search field.
...
Choosing a suggestion takes the user to that result. Where free-text search is supported, submitting the query takes the user to a results page. Keep the entered query available for refinement; follow [choosing and submitting a result](#choosing-and-submitting-a-result).
...
With the suggestion list open, pressing **Enter** chooses the active suggestion, which may be a different result after using the arrow keys. See [choosing and submitting a result](#choosing-and-submitting-a-result).
...
In addition to returning matching results in the menu, use free text search capability to allow users to enter a custom string and search across a broader result set.
...
When no suggestion is active, pressing **Enter** submits the typed query if the application supports free-text search. If a suggestion is active, Enter chooses it instead.
...
Users can move through suggestions with the arrow keys and press **Enter** to choose the active result, or click a result directly. Choosing a suggestion does not also submit a free-text query.
...
A search button can also be added as an optional element, providing a clear call to action and visual cue for more explicit search experiences.
...
The search pattern currently supports basic search result items that comprise non-focusable elements: text, icons, country symbols, and avatars.
...
<Callout title="Large data sets in search results">
  The Combo Box has limitations in efficiently handling large numbers of search
  results. For large data sets, consider methods like limiting the results or
  directing users to a separate search results page to enhance the user
  experience. This behavior is not currently supported. Contact the Salt team
  through the [support and contributions](/salt/support-and-contributions) page
  if it is important to your workflow.
</Callout>
