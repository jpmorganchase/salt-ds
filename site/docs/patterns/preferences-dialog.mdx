...
You should ideally house any editable settings within a centralized preferences dialog to display and manage user preferences across a platform, application or module. Displaying preferences should be contextual, i.e., the dialog should contain all preferences but depending on the entry point to the user interface, the preferences shown to you in the first instance will vary. 
...
<ExampleContainer type="positive" customPillText="Best practices">
  Display core actions to the right of a dialog. Refer to the button bar pattern for further guidance on how to display user actions within dialogs. 
</ExampleContainer>
...
The preferences dialog is responsive to the width of the application viewport and respond dynamically to the Salt breakpoints. 
...
For smaller viewports (i.e. devices with limited screen sizes), it's preferable to configure buttons to stack horizontally at the XS breakpoint. This ensures longer button labels accommodate mobile devices that may not fit within the horizontal layout. They also remain an adequate size for touch interactions.
