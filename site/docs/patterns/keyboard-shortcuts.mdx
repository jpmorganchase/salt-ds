...
- When the application does not provide keyboard shortcuts, do not show an empty shortcut reference pattern.
- When users need to perform an action directly in the interface, use the relevant button, menu item, link, or control instead; shortcuts must remain an alternative path.
- When a workflow is not fully operable through standard keyboard navigation, do not rely on shortcuts to provide keyboard accessibility.
- When a shortcut conflicts with essential system, browser, platform, or assistive-technology shortcuts and cannot be changed, scoped, or disabled, do not define that shortcut.
...
- [Dialog](salt/components/dialog): Suitable when users need to focus on shortcuts without distraction, and when shortcuts should be accessible from anywhere in the application through a keyboard shortcut—quickly and without disrupting common tasks.
-  [Preferences dialog](salt/patterns/preferences-dialog): Works well if keyboard shortcuts are part of broader settings.
- [Drawer](salt/components/drawer): Appropriate for persistent access, or when shortcut settings are relevant to a specific part of the application that is in focus.
