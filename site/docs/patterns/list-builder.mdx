...
- When the task is only single selection, use List box, Combo box, or Dropdown instead.
- When users only need to filter a list, use List filtering instead.
- When the list is very small, use a simple re-orderable list or another lighter pattern instead.
...
Display a message to prompt or instruct the user when a list is empty. You can customize this message. Make sure that the text is concise, descriptive and clearly understood. Use the [content status pattern](/salt/patterns/content-status) to display empty state messages owned by the list-builder workflow.
