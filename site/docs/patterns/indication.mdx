...
<LivePreview componentName="patterns/indication" exampleName="Status" />

## When to use

- Use status indication to communicate the state or condition of a system or process.
- Use sentiment indication to guide users with visual cues about potential outcomes.
- Use progression indication to show the stage of a process or workflow.
- Use urgency indication to communicate the level of risk associated with an item, process, or entity.

## When not to use

- When users need a detailed message, recovery instruction, or inline validation, use supporting text or the relevant feedback component instead.
- When the item needs a primary action, use an action component instead of relying on color, iconography, or status labeling.
- When users need to navigate between locations, use Navigation, links, or another wayfinding pattern instead.
- When the visual treatment is only decorative and does not communicate state, sentiment, progression, or urgency, do not use indication.

## How to build

- Choose the indication type that matches the meaning: status, sentiment, progression, or urgency.
- Apply the color, icon, and label guidance for the selected indication type.
- Use status for system or process conditions, sentiment for outcome cues, progression for workflow stages, and urgency for risk levels.
...
<LivePreview componentName="patterns/indication" exampleName="Progression" />
...
<LivePreview componentName="patterns/indication" exampleName="Urgency" />
