...
A selectable card allows users to make selections by clicking anywhere within the card. It’s a visual equivalent to a form control, such as [`ToggleButton`](salt/components/toggle-button), [`RadioButton`](/salt/components/radio-button) or [`Checkbox`](/salt/components/checkbox). You can group selectable cards to support single or multiple selection.
...
- By default, titles and supporting content are left aligned inside the card when using left-to-right languages like English. For text alignment alternatives, refer to the Salt [typography foundation](/salt/foundations/typography).
</ExampleContainer>
...
- Refer to the Salt [spacing foundation](/salt/foundations/spacing) for more guidance on spacing between cards.
</ExampleContainer>
