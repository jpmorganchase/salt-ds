...
Cards are most often arranged using a [grid layout](/salt/components/layout/grid-layout), which is ideal for larger amounts of screen real estate. Use a grid layout to display a large number of cards in an orderly manner, such as product listings, catalogues or galleries.
...
Use a [stack layout](/salt/components/layout/stack-layout) to compose cards vertically and in a single column, to showcase sequential or related content in a list format. This is ideal for interfaces with limited viewport width, such as mobile devices.
