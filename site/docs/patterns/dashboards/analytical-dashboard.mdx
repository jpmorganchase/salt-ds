...
- [**Border layout**](../../components/border-layout): Use the border layout component to manage the top-level layout of the dashboard, including header, footer and navigation panel elements.
- [**Flow layout**](../../components/flow-layout): Use for small-scale layouts that require components (such as cards) to be displayed in a responsive row.
- [**Stack layout**](../../components/stack-layout): Use for small-scale layouts that require displaying components in a single row or column and may benefit from the supported visual separators.
...
- **Responsive design**: Ensure that the dashboard is responsive and can adapt to different viewport sizes and devices. Use the [Grid Layout component](../../components/grid-layout) to manage the layout and responsiveness of the main content area in your dashboard.

## Designing a basic dashboard

The following guidance explains how to layout your own dashboard using [Salt foundations](/salt/foundations/index). Use this to understand how to apply spacing to create a consistent visual rhythm within your designs.

## Layout grid

Salt design system provides layout grids as a starting point for designing any user interface that achieves effective alignment and consistency for your dashboard design. View the [Salt Layout Grid library](<https://www.figma.com/file/Qun1kdSvMhq3gk9Bo6dOrC/Salt%3A-Layout-Grid-(Community)?type=design&node-id=16-3068&mode=design&t=l9ZRR3WVCS4BuHZA-0>) for grid templates across different densities.
...
- Use the [Border Layout](../../components/border-layout) component to manage the top-level layout of your dashboard.
- Use the [Drawer](../../components/drawer) component to display an expandable panel overlaid above a region of your dashboard rather than embedded within the existing layout. Users can view additional content without navigating to a different screen.
...
- The default padding between modules is spacing-300, but depending on the needs and business context of the dashboard, you can change this spacing token if your dashboard needs to display more or less content. View the [spacing foundation](/salt/foundations/spacing) for further guidance.
...
- _(JPM employees only)_ For styling data visualizations such as charts, follow the UITK guidelines on [Data Visualization](https://uitk.gaiacloud.jpmchase.net/design/data-visualization), where you can find further information on presenting data in a visually cohesive manner. We recommend using the [categorical color palette](https://uitk.gaiacloud.jpmchase.net/design/data-visualization/color#color-palettes:categorical-palette) within your dashboard design, as it’s ideal for visualizing distinct groups of items.
...
Metric bars should remain clean and concise to provide a higher level oversight of information. Try and adhere to the [Law of Proximity](https://lawsofux.com/law-of-proximity/) and avoid overloading this space with excessive information. Aim for no more than 3–6 key metrics on display, and group contextually related metrics together where possible.
...
- Layout is flexible depending on the dashboard grid. A metrics bar can span the full width of a dashboard (e.g., over 12 columns) or follow a [split layout](../../components/split-layout) using the remaining number of available columns across the main body of the dashboard. Use the [Stack Layout](../../components/stack-layout) or [Flow Layout](../../components/flow-layout) components to organize the layout within a metric bar depending on whether metrics should wrap or scroll within a module.
