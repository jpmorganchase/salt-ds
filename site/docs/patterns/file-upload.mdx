...
- When users only need to browse files or records that already exist in the product, use a list, table, or data grid surface instead.
- When users are entering a file URL, path, or other text reference without transferring a file, use an input field instead.
- When the main task is ongoing file management after upload, use a full-page layout or drawer workflow around the file list and actions instead of a standalone embedded upload area.
...
- Use the [`StackLayout`](/salt/components/layouts/stack-layout) to display the File drop zone, file upload items and text elements that make up file upload in a column.
- Use the [`StackLayout`](/salt/components/layouts/flex-layout) with a gap of 1 to position the text elements.
...
<Diagram
  ...
  alt="File upload with a group error message."
  border
/>
...
<Diagram
  ...
  alt="File upload with a group error message banner."
  border
/>
