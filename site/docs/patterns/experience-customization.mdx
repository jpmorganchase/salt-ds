...
<GuidanceCallout type="positive" customPillText="Best practices">
**Switch in wizard**: Switches can be used in multi-step wizards even when changes don’t take effect immediately. Use switches only for true on/off choices, and make any final behavior unmistakable—for example, label the final button 'Finish and apply changes' instead of just 'Finish'.
</GuidanceCallout>
