...
<LivePreview componentName="patterns/experience-customization" exampleName="StandardControls" />
...
- When choices are fixed by policy, role, permissions, or system logic and users cannot change them, do not present them as customization options.
- When a setting only affects the current task, page, or data view, use local controls in that workflow instead of onboarding or preferences.
- When users are entering data required to complete a business task, use the relevant form or workflow instead of the customization pattern.
- When options expose styling, component design, implementation details, or system logic, do not include them in user-facing customization.
...
**Language selection**: If you include language selection in this flow, place it at the beginning so users can understand the rest of the setup. Apply the selected language from the next step onward. When the language changes, ensure the language metadata is updated (set the lang attribute to a valid language code) so assistive technologies interpret the content correctly.
</GuidanceCallout>
...
<GuidanceCallout type="positive" customPillText="Best practices">
**Heading hierarchy**: If the live preview includes headings or titles, ensure they follow the heading hierarchy of the surrounding page or modal.
</GuidanceCallout>
