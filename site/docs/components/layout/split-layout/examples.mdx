...
The `direction` prop defines the main axis along which items are displayed. By default, `direction` is set to `row`, arranging items horizontally. Setting `direction` to `column` arranges items vertically along a single line.
...
The split layout component supports responsive props, so you can define different behaviors based on viewport size. You can use the default [Salt breakpoints](https://www.saltdesignsystem.com/salt/foundations/responsiveness/breakpoints) or pass in your own using `SaltProvider`.
