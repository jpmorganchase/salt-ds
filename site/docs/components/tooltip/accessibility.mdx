...
## What is a tooltip
A tooltip is a non-modal overlay that is hidden by default and displays text-only content on hover or focus. It should only be used to provide extra (non-essential) information about a [`Form Field`](../form-field), [`Link`](../link), [`Button`](../button), or other interactive element.
