...
The application or form library decides when a value is invalid. Set
`validationStatus="error"` to expose that result on participating controls through
`aria-invalid`; warning and success do not set this default invalid state. Supply
specific feedback through `FormFieldHelperText`. Explicit native `aria-invalid`
props remain available for overrides. See [Form integration](/salt/getting-started/form-integration)
for control-specific value, ref and submission bindings.
