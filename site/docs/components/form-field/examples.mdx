...
- Apply the prop to the field, not the control it contains, for expected results.
- Set the field to read-only if you’re displaying predefined, non-editable values, e.g., entry populated and fixed by template selection.
...
Wrap multiple form fields in [`StackLayout`](../layouts/stack-layout) to form a group:
...
Use multiple [stack layouts](../layouts/stack-layout) inside a group to create a multiple column layout.
...
Use [`GridLayout`](../layouts/grid-layout) within a group to create multiple columns where empty slots may be present.
