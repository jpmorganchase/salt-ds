...
The buttons will show a loading spinner when setting the isLoading prop to true. You should pass a message using the loadingText prop and `showLoadingText` as `true` for screenreader to announce while loading state is true. It defaults to "Loading".
