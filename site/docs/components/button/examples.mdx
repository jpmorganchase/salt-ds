...
**Accessibility** - Loading is a visual only state, therefore if you have additional accessibility requirements, you need to handle them manually. One way to do that is to announce a message to screen readers. This can be done by passing the `loadingAnnouncement` prop.
