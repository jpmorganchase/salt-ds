...
- **Solid surfaces only**: `OnSolidButton` renders light text on a transparent background, so it's only legible against a solid semantic fill. On a default page surface it will be close to invisible.
