...
{/* TODO: This section documents a release candidate */}

Use an `OnSolid` button to deliver a consistent, accessible action style on solid semantic surfaces, most commonly for close or dismiss actions within status messages that use solid fills, ensuring the button maintains required contrast regardless of the underlying semantic color.

`OnSolid` is currently a release candidate available from the `@salt-ds/lab` package:
