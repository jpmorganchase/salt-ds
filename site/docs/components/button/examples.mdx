...
Use an `OnSolidButton` button to deliver a consistent, accessible action style on solid semantic surfaces, most commonly for close or dismiss actions within status messages that use solid fills, ensuring the button maintains required contrast regardless of the underlying semantic color.

`OnSolidButton` is currently a release candidate available from the `@salt-ds/lab` package:
...
- **Solid surfaces only**: `OnSolidButton` renders light text on a transparent background, so it's only legible against a solid semantic fill. It will not provide enough contrast to be reliably visible against container backgrounds (primary/secondary/tertiary).
- **Low emphasis**: Reserve it for supporting actions such as dismiss or close. For the primary action on a solid surface, use a button with an appearance that provides more prominence.

<LivePreview componentName="button" exampleName="OnSolid" />
