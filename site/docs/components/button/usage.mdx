...
- To allow the user to execute an action, such as “submit”, “merge” or “upload”. Buttons are typically placed in components and contexts such as [Dialog](/salt/components/link) and [Forms](/salt/components/formfield).

#### When not to use Button

- When the user is navigating to a new page or window. Instead, consider [Link](/salt/components/link). The exception is if the navigation is the core action on the page. In this case, use a CTA button.
