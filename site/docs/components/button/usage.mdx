...
- Use buttons to communicate actions that a user can perform, such as “submit”, “merge”, “upload” etc. Buttons are typically placed in components/contexts such as Dialog and Forms.

#### When not to use Button

- Consider a Link instead of the Button component if the user is navigating to a new page or window rather than carrying out an action. The exception to this behaviour is if the navigation is the core action on the page. In this case, a CTA button should be used to trigger the navigation.
