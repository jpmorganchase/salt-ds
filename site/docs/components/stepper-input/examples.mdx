...
**Note:** Stepper input's role is changed to `"textbox"` when read-only is set to work around browser/screen reader limitations when conveying a read-only state.
