...
- Holding down the Shift key when pressing Up Arrow or Down Arrow key changes the value by the amount defined by the `block` prop.

### Best practices

- Select a `step` value that aligns with the precision required by the use case.
- Set the `block` value to a reasonable size compared to the `step`, allowing users to make larger adjustments.
