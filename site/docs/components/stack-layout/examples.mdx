...
The stack layout `direction` prop defines the main axis along which items are displayed. By default, `direction` is set to `column`, arranging items vertically. Setting `direction` to `row` arranges items horizontally.

The values `row-reverse` or `column-reverse` can be used to reverse the order of items along the main axis, displaying them in the opposite
