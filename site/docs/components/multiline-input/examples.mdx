...
Multiline Input can be set to a disabled state, with no resultant action when the user interacts with it. Showing it in a disabled state allows the user to be aware of its presence or an entered value while restricting their ability to edit it.
...
You can use a [Button](/salt/components/button) at the beginning or end of the Multiline Input using the `startAdornment` and `endAdornment` props. Buttons can allow for custom interactivity within the Multiline Input area itself, or elsewhere in the app via the Multiline Input.

Ensure to make your interactive buttons disabled or read-only should it match the state of the containing Multiline Input.
