...
You can use a [Button](/salt/components/button) the beginning or end of the Multiline Input using the `startAdornment` and `endAdornment` props. Buttons can allow for custom interactivity within the Multiline Input area itself, or elsewhere in the app via the Multiline Input.

Ensure to make your interactive Buttons disabled or readonly should it match the state of the containing Multiline Input.
