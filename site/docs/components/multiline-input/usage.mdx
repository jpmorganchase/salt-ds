...
A Multiline Input is recommended when a user is asked a subjective question and the length of their answer is unknown. The input can give them plenty of space to answer the question and review the answer. Set the height accordingly if you can take an educated guess at the possible length of their answer–see the [Number of Rows example](../examples#number-of-rows).
