...
To add non-numeric `marks` to certain values, please refer to the [example with non-numeric values](/salt/components/slider/range-slider/examples#with-non-numeric-values) below.
