...
By default the minimum and maximum values are shown, and can be displayed either inline or below the track (using the `labelsPosition` prop).
...
Selection of a range of values from a horizontally oriented slider track. This determines a minimum and maximum range. Values can be displayed either inline or below the track. Thumbs cannot pass or overlap.
...
- Only use the inline variant of slider alongside an input. This ensures the size-base alignment of the overall component layout is maintained.
  ...
  - Markers will be displayed below the slider track so ensure you set `labelPosition` to `bottom` to have consistent labels.
