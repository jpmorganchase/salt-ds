...
Tooltip displays the value on handle focus. Tooltip remains persistent on cursor drag or moving the handle along the track with keyboard interaction. By default, tooltip is configured to flip to switch placement sides if constrained by space, so that it remains visible.
