---
title:
  $ref: ./#/title
layout: DetailComponent
sidebar:
  exclude: true
data:
  $ref: ./#/data
---

You should always position the Banner component in the main content area, directly below the navigation, or in the related container component.

You should configure it to close when:

- The user performs an action that fixes an issue. For example, they correct a validation error.

- The system status that triggered the notification is resolved. For example, a server connection is restored.

### When to use Banner

- To show a notification that applies to the user’s current task or workflow.

### When not to use Banner

- Do not use Banner to notify users of an event that has occurred in a peripheral application or workflow, consider using Toast.

- If the notification is caused by a low priority event not requiring user feedback, consider using Toast.

Content

A Banner is typically used with one or two lines of text; therefore, content should be short, clear, and concise, allowing users to quickly scan the notification and understand the situation and/or next steps.

The content is set to Body Default, however, if titles are needed, they should be configured to display the Body Strong typography style across densities.

### Import

To import Banner from the core Salt package, use:

```
import { Banner } from "@salt-ds/core";
```

### Props

<PropsTable componentName="Banner" />
