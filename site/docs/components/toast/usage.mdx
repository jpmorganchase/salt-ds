...
Titles should be configured to display the Body Strong typography style across density. Because Toast has limited space, content should be clear and concise allowing users to quickly scan the notification and understand the situation and/or next steps.  

#### Custom content 

You can modify Toast to contain custom content such as actionable components, like Buttons. This allows the user to act on the message without interrupting their current workflow (for example, users can approve or respond to an update without opening a new window). Users should always be able to close or dismiss the Toast.  

Keep custom content relatively short where possible. If the custom content or action within the Toast is important enough to interrupt the user’s current task, consider using a Dialog instead.  
