...
- When the checkbox displays a mutually exclusive choice between two or more options. Instead, use [Radio Button](../components/radio-button).
- To display a single option but trigger a state change directly and immediately. Instead, use [Switch](../components/switch).
