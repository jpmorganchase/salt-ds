...
By default, `SidePanel` uses its header content as the accessible name for the panel region.  
Override this when needed with `aria-labelledby` (preferred when a visible heading exists) or `aria-label` (when no visible heading is available).
