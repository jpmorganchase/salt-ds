...
<SidePanelProvider>
  <FlexLayout>
    ...
    <SidePanel position="right" aria-labelledby={"heading id"}>
      <Text>Side panel content goes here.</Text>
      <SidePanelContent header={<H2 id={"heading id"}>Details</H2>}>
        <Text>Side panel content goes here.</Text>
      </SidePanelContent>
    </SidePanel>
...
```jsx
const { setOpen, setReference, getReferenceProps, getFloatingProps } =
  useSidePanelContext();

<Button
  {...getReferenceProps({
    "aria-controls": getFloatingProps().id,
    onClick: () => {
