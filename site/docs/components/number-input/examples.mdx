...
The `NumberInput` is designed with a default increment step of 1, which is adjustable via the `step` prop for precise value changes. For larger adjustments, users can hold the Shift key while using the Arrow keys to apply a `stepMultiplier`, a prop that defaults to 2.
...
The `decimalScale` prop controls how many decimal places are allowed. By default, the `decimalScale` is auto-generated using the number of decimal places in the `step` or `defaultValue` or `value` prop, whichever is greater, however, you can set it to any value of your choice.
