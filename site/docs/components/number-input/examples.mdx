...
The `NumberInput` allows you to format its value using the `format` callback prop. This typically works in conjunction with a `parse` callback which determines the numerical value, as well as an `isAllowed` callback to control which values can be entered.
...
- **`isAllowed`**: This callback returns a boolean as to whether the entered value is valid or not. This is particularly useful for restricting input to specifi ranges, patterns, or formats. For example, you can prevent users from entering invalid characters or values outside a defined range.
