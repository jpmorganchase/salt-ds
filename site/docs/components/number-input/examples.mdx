...
## Customise input value

By default, any numeric decimal value, with an optional sign prefix can be entered, such as `+123.456`.
To support such use-cases as `10x` an `isAllowed` callback can be provided to create custom forms of entry.
...
To provide a fixed scale provide a custom format function.

<LivePreview componentName="number-input" exampleName="FixedDecimalScale" />
...
Large numbers can be transformed into their compact forms to be used in, for example, charts or other data visualizations. You can use `format` to convert the numerical value into its shortened form and `parse` to convert the shortened form back to the numerical value. The example below supports the following conversion:
