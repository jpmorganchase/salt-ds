...
By default, when `min` and `max` values are set and user types a value that is out of range, the value is not clamped. You can set the `clampBehaviour` to `default` to clamp the value to `max` or `min`, whichever is closest when the input loses focus (on blur). Setting the `clampBehavior` to `strict` will not allow the user to even type an out of range value.
...
The `decimalScale` prop controls how many decimal places are allowed. By default, the `decimalScale` is auto-generated using the number of decimal places in the `step` or `defaultValue` or `value` prop, whichever is greater.

When `clampBehaviour` is set to `strict` mode, you can not type more than the allowed decimal places.
...
With Intl Number Format, start and end adornments and custom formatting.
