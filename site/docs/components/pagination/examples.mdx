...
Truncation, represented by ellipses, occurs when it’s not possible to display all pages within a range. It enables you to accommodate a large set of results without displaying every page.
...
You can configure the visible page range using the `boundaryCount` prop—increasing or decreasing the number of pages displayed next to the arrow buttons. Minimum value is 1, so there will always be at least one item next to the arrow buttons.
...
Example of custom combination of CompactPaginator and GoToInput.
