...
Place the component close to the page content. Consider whether you should place it above, below, or both above and below the related content.
...
Use `CompactPaginator` with the `withInput` prop to provide the ability to jump to a specific page number.
...
- Consider the number of results likely to be displayed and the available space, when configuring the width of your Pagination. Although truncation helps minimize its width, a range containing 100s or 1000s of pages will inevitably be wider than a range containing two-digit pages.
...
You can configure the visible page range using the `siblingPairCount` prop—increasing or decreasing the sibling range, i.e., pages on either side of the active page, depending on the breakpoint. Always make changes to sibling pages in sets of two to ensure that your page range is an odd number. This keeps the active page centred within the range and visually balanced.
...
This is an example of a custom combination of `CompactPaginator` and `GoToInput`.
