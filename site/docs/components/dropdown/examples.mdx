...
`DropdownNext` contains a button that opens a list on mouse or keyboard interaction.

### Best practices

You can wrap `DropdownNext` in a `FormField` when it’s displayed within a form context. This provides functionality built into `FormField` for increased accessibility. For more information, refer to the [`FormField`](../form-field) guidance.
...
When the `value` prop is an object, the `valueToString` prop must be provided to `ComboBoxNext` to specify how to display the option in the button.
