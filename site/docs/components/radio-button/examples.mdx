...
Use a disabled state for radio buttons that have permissions, dependencies or pre-requisites. For example, a radio button in a [Form](/salt/components/formfield) may be disabled because the user has not yet completed an earlier section of the form.
...
Read-only radio buttons are navigable using keyboard shortcuts. This means that, unlike disabled radio buttons, users can interact with the text description. Use a readonly radio button instead of a disabled radio button when the text description contains information that is valuable to the user.
