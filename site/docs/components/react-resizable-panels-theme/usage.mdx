...
For panel resize handles, add the class name `resize-handle-salt-border-<border>` to the `PanelResizeHandle` component, to add borders to the resize handlers. Add the `resize-handle-salt-variant-<variant>` class to set the background color to one of the salt container background colors.
