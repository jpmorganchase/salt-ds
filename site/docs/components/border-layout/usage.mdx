---
title:
  $ref: ./#/title
layout: DetailComponent
sidebar:
  exclude: true
data:
  $ref: ./#/data
---

### Using the component

### When to use

- To manage the top-level layout of your application, or page, with at least one additional region alongside the main content area (e.g., for navigation or a footer).

### When not to use

- When you're not managing the top-level layout of your application, but the layout inside one of the five border layout regions.

- For a customized layout that you can control in both row and column dimensions (for example, a dashboard). Instead, use [`GridLayout`](../components/grid-layout).

- For a side panel that temporarily reveals relevant content. Instead, use [`Drawer`](../components/drawer).

### Import

To import `BorderLayout` and `BorderItem` from the core Salt package, use:

```
import { BorderItem, BorderLayout } from "@salt-ds/core";
```

### Props

#### `BorderLayout`

<PropsTable componentName="BorderLayout" />

#### `BorderItem`

<PropsTable componentName="BorderItem" />
