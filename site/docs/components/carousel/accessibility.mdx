...
<GuidanceCallout type="positive" customPillText="Best practices">
  - Avoid using infographics, maps, or charts as images, and do not use [images
  of
  text](https://www.w3.org/TR/UNDERSTANDING-WCAG20/visual-audio-contrast-text-presentation.html).
  - Prioritize slides by placing the most critical information first. - Tablist
  components offer keyboard navigation, which may not be ideal for mobile
  devices where space is limited and users prefer gesture-based navigation.
</GuidanceCallout>
...
- Refrain from using progress dots on mobile devices, as space is limited and users favor gesture-based navigation.
