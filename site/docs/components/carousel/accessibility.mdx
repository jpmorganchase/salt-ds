...
- consider requiring user consent to enable autoplay, especially for users who may find automatic movement disorienting.
  - permantly pause the autoplay when the user hovers over the carousel.
...
- When the focus is on the next button, shift the focus to the tablist (if available). If the tablist is not present, move the focus to the first visible carousel slide that contains a focusable element. If neither option is available, direct the focus to the next focusable element in the sequence of the page. If there are no focusable elements within the slide, focus moves to one of the carousel controls.
...
<KeyboardControl keyOrCombos="Shift + Tab">
 ...
 - If focus is within a visible slide, moves focus to the tablist (if available) or next button, if positioned above the slide, ensuring logical navigation order. If there are no focusable elements within the slide, focus moves to one of the carousel controls.
