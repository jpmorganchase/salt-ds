...
- If composing with navigation controls (including tablist), ensure they are the first element to receive focus, enabling them to control the carousel, upon focus.

### With tablist

If composing with a tablist:

- If composing with a tablist, ensure they are the first element to receive focus, enabling them to control the carousel, upon focus.
...
- If multiple cards are visible, within one slide, do not include a tablist, as a tablist should only be used with single cards.
- If using a tablist, ensure each card has`role="tabpanel"`.

### Without tablist

If composing without a tablist:

- If not using a tablist, ensure each card has`role="group" and `aria-roledescription="slide".
