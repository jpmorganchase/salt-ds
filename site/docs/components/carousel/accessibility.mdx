...
- If composing with navigation controls (previous/next slide or tablist), ensure they are the first elements to receive focus, enabling the user to control the carousel.
...
- If multiple cards are visible at one time, do not include a tablist, as a tablist should only be used with single cards.
- Ensure each card has `role="tabpanel"` and `aria-roledescription="slide".

### Without tablist

If composing without a tablist:

- Ensure each card with a carousel has `role="group"` and `aria-roledescription="slide"`.
