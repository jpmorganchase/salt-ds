...
The carousel can display slides at a constant width regardless of viewport size. Set the required Embla props, then set `--carousel-slide-size` to a value (e.g., `550px`) to prevent the slides from scaling with the viewport.

When `--carousel-slide-size` is not overridden, the default responsive behavior is preserved.

<LivePreview componentName="carousel" exampleName="FixedWidth" />
...
You can render fully custom slide content, such as `Avatar` components, by composing your own slide elements inside `CarouselSlides`.

<LivePreview componentName="carousel" exampleName="CustomCarouselSlide" />
