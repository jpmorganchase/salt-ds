...
The carousel can display slides with a fixed width that remains constant regardless of viewport size by passing `fixedSlideWidth` prop a CSS length value or number (treated as pixels).

When `fixedSlideWidth` is not set, the default responsive behavior is preserved.
