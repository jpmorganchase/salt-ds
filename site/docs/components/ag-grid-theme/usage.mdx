---
# Leave the frontmatter as is
title:
  $ref: ./#/title
layout: DetailComponent
sidebar:
  exclude: true
data:
  $ref: ./#/data
---

### Using AG Grid Theme

#### When to use AG Grid Theme

 - When you want to use the UI Toolkit V1 theme. Use `UITK AG Grid theme`.
 - When you want to use the Salt theme. Use `Salt AG Grid theme`.

#### When not to use AG Grid Theme

When you want to use a component that is not related to AG Grid. Instead, use Grid.

### Import

{/* Update the text and code snippet below as needed: */}

To install the `@salt-ds/ag-grid-theme` package, run the command appropriate to your environment:

```js
yarn add @salt-ds/ag-grid-theme
```

```js
npm install @salt-ds/ag-grid-theme
```

#### Import UITK AG Grid theme

To import the UITK AG Grid theme, use:

```js
import '@salt-ds/ag-grid-theme/uitk-ag-theme.css';
```

For light mode, use these CSS class names:

```js
.ag-theme-uitk-high-light
.ag-theme-uitk-medium-light
.ag-theme-uitk-low-light
.ag-theme-uitk-touch-light
```

For dark mode, use these CSS class names:

```js
.ag-theme-uitk-high-dark
.ag-theme-uitk-medium-dark
.ag-theme-uitk-low-dark
.ag-theme-uitk-touch-dark
```

#### Import Salt AG Grid theme

To import the Salt AG Grid theme, use:

```js
import '@salt-ds/ag-grid-theme/salt-ag-theme.css';
```

For light mode, use these CSS class names:
```js
.ag-theme-salt-high-compact-light
.ag-theme-salt-high-light
.ag-theme-salt-medium-light
.ag-theme-salt-low-light
.ag-theme-salt-touch-light
```

For dark mode, use these CSS class names:
```js
.ag-theme-salt-high-compact-dark
.ag-theme-salt-high-dark
.ag-theme-salt-medium-dark
.ag-theme-salt-low-dark
.ag-theme-salt-touch-dark
```

For variants, use these CSS class names:
```js
.ag-theme-salt-variant-secondary
.ag-theme-salt-variant-zebra
```

For editable fields, add the class name `.editable-cell` to the `cellClass` array in your column definition.

For fields displaying numeric values, add the class name `.editable-cell` to the `cellClass` array in your column definition.
