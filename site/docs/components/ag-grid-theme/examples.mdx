...
Rows can be selected via checkboxes.
...
Cells, rows, or columns can have color applied to them.
...
The menu can be opened on the column to view details or filters.
...
HD Compact theme is available by applying the `ag-grid-salt-high-compact` classname.
