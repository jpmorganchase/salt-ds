...
If you are deciding between `Link`, `Button`, or a custom interactive element,
start with [Choosing the right primitive](/salt/getting-started/choosing-the-right-primitive).

<Callout title="Quick check">

- Use `Link` when the element has a real destination such as a route, page,
  document, or page section.
- Do not use `Link` as a generic action trigger with only `onClick`. Use
