...
To enable a user to interact with the steps, provide a `render` function to replace the label with a [Link](../link).

<LivePreview componentName="stepped-tracker" exampleName="HorizontalWithLinks" />

## Vertical With Links

To enable a user to interact with the steps, provide a `render` function to replace the label with a [Link](../link).

<LivePreview componentName="stepped-tracker" exampleName="VerticalWithLinks" />

## Hook

The `SteppedTracker` and `Step` component are primarily presentational components.
