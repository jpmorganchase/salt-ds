...
`TD` and `TH` can be truncated using the `maxRows` prop. The `maxRows` prop accepts a number that specifies the maximum number of rows to display before truncating the text.

<LivePreview componentName="table" exampleName="Truncation" />
