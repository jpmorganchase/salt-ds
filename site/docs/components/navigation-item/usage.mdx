---
title:
$ref: ./#/title
layout: DetailComponent
sidebar:
exclude: true
data:
$ref: ./#/data
---

### How to use

#### When to use Navigation Item

- Use Navigation Item when you have multiple levels of navigation in your application or website.
- We suggest first considering a horizontal group of Navigation Items for distinct webpages or applications. If you have multiple (more than eight) services to navigate to at a platform level, or have navigation across distinct apps, consider using a vertical group instead.
- If you need your items to remain visible on scroll, we recommend displaying Navigation Items in a fixed App Header (or header area)

#### When not to use Navigation Item

- When you need to show different views of content within the same page, consider using Tabs instead.

- When your content is very specific. In this case, a minimalistic or single-page design may be more appropriate than a complex navigation structure. Consider using the [Interactable Card](), or a [Link]() component within the body of the page.

### Import

To import Badge from the lab Salt package, use:

```
import { NavigationItem  } from "@salt-ds/core";
```

### Content

To make the most of screen real estate, oOur recommendation is that all description text should be clearshort and concise.

Always use write descriptions in sentence case in a Navigation Item.

When using a Badge component within a Navigation Item, we recommend using sentence case.

In a Horizontal Navigation Item, do not wrap descriptions, use truncation with Tooltip if necessary.

In a Vertical Navigation Item, descriptions can wrap onto multiple lines if necessary, and is preferred over truncation.

### Props

<PropsTable componentName="NavigationItem"/>
