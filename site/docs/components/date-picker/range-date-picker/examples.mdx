...
When using a modal `DatePicker` with confirmation controls, set the `enableApply` prop to `true` to enable apply mode. This ensures that date range selections are not committed until the user explicitly applies them by clicking the Apply button. This is useful when you want to validate selections or allow users to cancel their changes before confirming.
