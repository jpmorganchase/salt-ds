...
<LivePreview
  componentName="date-picker"
  exampleName="SingleWithDisabledDates"
/>
...
## Single with unselectable dates
...
To customize the rendering of a day element inside a panel, such as `SingleGridPanel`, pass the `renderDayButton` prop through to the `Calendar` component using the `CalendarGridProps`. This prop accepts a render function that is passed the default props, status and date. This can be useful for scenarios where you need to display additional information or apply custom styling to specific dates.

<LivePreview
  componentName="date-picker"
  exampleName="SingleWithCustomRendering"
/>
