...
A composable API enables you to use provided child components, such as `DatePickerSingleInput` and `DatePickerSingleGridPanel`.
...
To import `DatePicker` from the Salt date components package, use:
...
<PropsTable packageName="date-components" componentName="DatePicker" />
...
<PropsTable packageName="date-components" componentName="DatePickerOverlay" />

`DatePickerOverlay` is the container component for the `DatePicker` overlay.

### DatePickerActions

<PropsTable packageName="date-components" componentName="DatePickerActions" />

`DatePickerActions` provides the controls to apply or cancel a date selection.

When using a modal `DatePicker` with confirmation controls, you must explicitly set `enableApply={true}` on the `DatePicker` component to enable apply mode. This ensures that date selections are not committed until the user explicitly applies them through the confirmation controls (e.g., Apply button).

If you are creating your own confirmation controls instead of using `DatePickerActions`, you must also explicitly set `enableApply={true}` on the `DatePicker` component.

### useDatePicker

The `useDatePicker` hook provides `state` and `helpers` to manage the `DatePicker` state.

<PropsTable packageName="date-components" componentName="useDatePicker" />
...
<PropsTable
  packageName="date-components"
  componentName="useDatePickerOverlay"
/>
