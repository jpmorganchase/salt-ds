...
The primary purpose of the `SystemStatus` is to provide information and, if necessary, prompt further action.
...
- For inline messaging. Instead, use `Banner`.
- To show a notification that applies to the user’s current task or workflow. Use `Banner` instead.
...
A `SystemStatus` typically has one or two lines of text. Make content short, clear, and concise, allowing users to quickly understand the situation and/or next steps.
...
To import `SystemStatus` and related components from the core Salt package, use:

```
import { SystemStatus, SystemStatusContent, SystemStatusActions } from "@salt-ds/core";
```
