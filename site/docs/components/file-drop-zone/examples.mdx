...
<LivePreviewControls>
  ...
  <LivePreview componentName="file-drop-zone" exampleName="Error">
    ...
    If uploaded files fail to meet the validation criteria, change `state` prop to `error` so the file drop zone will display the error state.
  ...
  <LivePreview componentName="file-drop-zone" exampleName="Success">
    ...
    If files uploaded successfully, change `state` prop to `success` so the file drop zone will display the success state.
