...
To enable multiple selection of single dates, set the `multiselect` prop to `true`.
...
_Multiple selection of date ranges is still work-in-progress, refer to our [roadmap](https://github.com/jpmorganchase/salt-ds/issues/4081) for details._
...
Disabled dates are not selectable. To implement this, use the `isDayDisabled` prop to pass a function that determines whether a day is unselectable and provides the tooltip description.
