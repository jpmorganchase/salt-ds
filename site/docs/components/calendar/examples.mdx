...
To customize the rendering of a day element, use the `renderDayButton` prop with the `Calendar` component. This prop accepts a render function that is passed the default props, status and date. This can be useful for scenarios where you need to display additional information or apply custom styling to specific dates.
