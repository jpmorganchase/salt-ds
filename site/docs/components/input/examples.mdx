...
- Tip: Input is responsive to the size of its parent container, filling it 100%. To disable responsiveness, apply a fixed width and height using the CSS API. 
...
Tip: For recommendations on variant choice, see the [Forms pattern](salt/patterns/forms). 
...
- Tip: Use the read-only state when the value is necessary for the users flow or current task but cannot be edited, for example, because of user permissions. 

</LivePreview>
<LivePreview componentName="input" exampleName="EmptyReadonlyMarker" >

### Empty Readonly Marker
...
You can use a [Button](/salt/components/button) the beginning or end of the Input field using the `startAdornment` and `endAdornment` props for an interactive Button within the field. 
...
- Tip: Numeric values are often displayed right aligned to make them easier to compare. However, sometimes number fields are unrelated or are mixed with text fields in a form, so left-alignment may promote better visual flow. Consider your use case when deciding on Input field alignment. 
