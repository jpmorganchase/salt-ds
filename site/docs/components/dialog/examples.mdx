...
Use `disableDismiss` to prevent accidental dismissal by clicking outside the dialog. Escape still closes the dialog and returns focus to its trigger; this prop does not require the user to confirm the action.

In this destructive example, `initialFocus` points to Cancel so the [documented focus order](./accessibility#focus-sequence) is preserved when the actions stack.
