...
Use the `represents` prop to describe what the Avatar represents, rather than describing its shape:

- `represents="person"` (default) renders a circular Avatar and falls back to a person icon.
- `represents="business"` renders a square Avatar and falls back to a business icon.

<LivePreview componentName="avatar" exampleName="Represents" />
