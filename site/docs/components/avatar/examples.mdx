...
Use `kind="person"` when the identity is a single individual (e.g., user profile, author, assignee, commenter). If you need to represent multiple people together, use `AvatarGroup`.
...
Use `kind="entity"` when the identity is a named non-person entity such as a team, business, organization, company, or account. Use an Entity avatar when representing the team as a single named identity and `AvatarGroup` when representing the team’s members.
