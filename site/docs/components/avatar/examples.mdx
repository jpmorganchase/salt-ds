...
Both `Avatar` and `AvatarGroup` can be made interactive using the `render` prop. Props defined on the JSX element will be merged with props from the component.
...
- Ensure the Avatar or Avatar Group has a meaningful accessible name so screen reader users understand what action will occur.
