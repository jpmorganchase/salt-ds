...
Use the `kind` prop to describe what the Avatar depicts, rather than describing its shape:

- `kind="person"` (default) renders a circular Avatar and falls back to a person icon.
- `kind="entity"` renders a square Avatar and falls back to an entity icon.

<LivePreview componentName="avatar" exampleName="Kind" />
