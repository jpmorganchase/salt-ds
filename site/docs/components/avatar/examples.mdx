...
Selecting the group opens a menu that displays the full list of members (including any hidden users), along with any supporting content such as labels or actions.
...
- The Avatar Group is a single interactive target; individual avatars within the stack are not interactive.
- Ensure any overflow indicator appears as the final avatar when used, and that it functions as an indicator—not a separate trigger.

### Visible avatars

Avatar Group doesn’t have a fixed maximum—its visible count is dynamic and, as space shrinks, avatars collapse into the menu one at a time with the overflow indicator updating. In limited space, aim for at least 2 visible avatars; if only one fits, use a single Avatar (Person or Entity) instead of an Avatar Group.

### Best practices

Avatar Group is typically displayed alongside other actions (for example, in toolbars, headers, or list row actions). For consistency and alignment, we recommend using the default size (1x) and not scaling an Avatar Group beyond the default multiplier.
