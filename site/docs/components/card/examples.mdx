...
Use `CardHeader`, `CardContent` and `CardFooter` to create distinct padded sections within `Card`, `LinkCard` or `InteractableCard`. Sections must be direct children of the card; don't wrap them in fragments or other components. When a section is present, the card removes its own padding and the sections provide consistent inner spacing. Adjacent sections share a single `--salt-spacing-200` gap.

`CardHeader` and `CardContent` don't impose an inner layout, so compose their children with Salt layout components as needed. `CardFooter` arranges its children in a horizontal row. Footers can contain actions in a default card, but must contain only static content in link and interactable cards.
