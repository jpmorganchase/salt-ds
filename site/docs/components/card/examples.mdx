...
Cards support strong, default, and subtle border colors, as well as no border. Cards use the default border token for their variant when `borderColor` isn't set. Setting `borderColor="none"` makes the border transparent.

- Use `borderColor="strong"` for higher-priority client experiences or when a stronger structural boundary is needed.
- Use the default border color for most cards unless there is a specific reason to increase or reduce visual emphasis.
