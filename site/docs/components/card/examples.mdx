...
Use `CardHeader`, `CardContent` and `CardFooter` to create distinct padded sections within `Card`, `LinkCard` or `InteractableCard`. When a section is present, the card removes its own padding and the sections provide consistent inner spacing. Adjacent sections share a single `--salt-spacing-200` gap.
