...
Cards are available in several variants: primary, secondary, and tertiary for all cards, with an additional ghost variant for static cards. These variants can be combined to create different levels of hierarchy within your UI. There is no strict order to follow; the choice largely depends on the content, layout, and structure of your UI. Select the variant that best aligns with the design language you are establishing and the hierarchy you aim to create.
