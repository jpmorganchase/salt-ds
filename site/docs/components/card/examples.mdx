...
- Leave `borderColor` unset for most cards unless there is a specific reason to increase or reduce visual emphasis.
...
Sections must render as direct children of `Card`, `LinkCard`, or `InteractableCard`; don't add an extra DOM wrapper around them. Place other direct children, such as images, between sections when content needs to extend to the card edges.

<LivePreview componentName="card" exampleName="Sections" />

## Collapsible card

Use `Collapsible` to show or hide a card's content while keeping its header and footer visible. Set `render={<CardContent />}` on `CollapsiblePanel` to merge both components onto one root element, preserving Card's direct-child section styling. To collapse only part of the content, render `CollapsiblePanel` normally inside `CardContent` instead.

<LivePreview componentName="card" exampleName="CollapsibleCard" />
