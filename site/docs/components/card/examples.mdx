...
Use `CardHeader`, `CardContent` and `CardFooter` to create distinct padded sections within `Card`, `LinkCard` or `InteractableCard`. When a section is present, the card removes its own padding and the sections provide consistent inner spacing. By default, adjacent sections share a single `--salt-spacing-200` gap.

Sections must be direct children of the card; don't wrap them in fragments or other components.
...
When using images in cards, set the image to span the card's entire width. You can do this by using the `CardContent` component inside `Card`, `LinkCard` or `InteractableCard`. `CardContent` removes the wrapping card's default padding, allowing siblings (such as images) to extend to the card's edges, and applies its own padding to the content inside it.
