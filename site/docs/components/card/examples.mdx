...
Use a ghost card to integrate static content with a custom background, image, or gradient without creating a strongly compartmentalized layout.
...
The `hoverable` prop adds hover styling to a static `Card`; it doesn't add interaction, semantics, or keyboard behavior. Use `LinkCard` or `InteractableCard` when the entire card is interactive.
