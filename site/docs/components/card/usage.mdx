...
A default card can contain any UI elements, including actionable components such as [`Button`](../button) or [`Link`](../link). You can use these, for example, to link to more detailed information.

Interactable cards shouldn't contain actionable components (like [`Button`](../button) or [`Link`](../link)), as the card itself is the interactive element.
