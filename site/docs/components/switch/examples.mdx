...
A read-only switch allows the user to select and copy its text, but doesn't allow them to change its state. You can set a switch as read-only via the prop `readOnly`.

**Note:** We recommend avoiding read-only switches due to the lack of adequate `aria-readonly` support and conflicting user expectations. If you must use a read-only switch, you should always add an accessible name via a visual label or an `aria-label` to make its name clear.
