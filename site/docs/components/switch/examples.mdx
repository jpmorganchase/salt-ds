...
**Note:** We recommend avoiding read-only switches due to the lack of adequate `aria-readonly` support and conflicting user expectations. If you must use a read-only switch, use a form field and add “(Read-only)” to the label to make its state clear.
