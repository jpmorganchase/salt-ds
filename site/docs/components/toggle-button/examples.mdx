...
- **Icon switching**: Consider switching icons based on the selection status of the toggle button, for instance by switching between outline and solid icon variants or active/inactive icons, such as locked and unlocked.
...
You can disable toggle buttons if required by context or user permissions using the `disabled` prop. Disabling a toggle button will prevent the user from interacting with it and suppress all functionality.
...
## Toggle button group selected appearance
...
- **Set Appearance on the Group**: Although its possible to mix sentiment within a group, its not recommended to do the same with appearance as it can lead to visual inconsistency and diminish clarity for the user.
