...
- Consider switching icons based on the selection status of the toggle button.
...
- Consider changing the text based on the toggle button's selection status, and ensure that any changes in the button's width do not adversely affect alignment.
...
Toggle buttons offer two visual styles to indicate a selected state: `solid` and `bordered`. The default is `solid` but you can set it to `bordered` using the `appearance` prop.
