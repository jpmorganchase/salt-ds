...
`ToggleButton` enables users to seamlessly switch between two distinct states, such as on/off or active/inactive.

The most commonly used sentiments are `neutral` and `accented`. Neutral provides less visual emphasis in the selected state, while accented provides more.
...
- **Self-explanatory icons**: An icon-only toggle button should be intuitive, ideally using well-known recognizable symbols. If the purpose of the button isn’t clear from the icon alone, add a label.
...
Switching icons based on the selection state of the toggle button can help communicate the current state of the button.

You can either switch between icon variants like the favorite example below or switch icons entirely like the lock/unlock example below.

<LivePreview componentName="toggle-button" exampleName="IconSwitching" />
...
- **Label switching**: Consider changing the text on the toggle button to complement its selected state. If the label does change based on the selected state, ensure the button's width does not change.

<LivePreview componentName="toggle-button" exampleName="TextOnly" />

## Sentiment

In addition to `neutral` and `accented`, `ToggleButton` has three other sentiments available — `positive`, `negative` and `caution`. You can set this using the `sentiment` prop.
...
By default, toggle buttons are set to `solid`. You can change the appearance using the `appearance` prop.

<LivePreview componentName="toggle-button" exampleName="AppearanceSolid" />

## Appearance - bordered

You can change the appearance using the `appearance` prop.

<LivePreview componentName="toggle-button" exampleName="AppearanceBordered" />
