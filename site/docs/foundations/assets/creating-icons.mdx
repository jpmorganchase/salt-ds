...
The [Icons/Icon/Icon QA](https://github.com/jpmorganchase/salt-ds/blob/main/packages/icons/stories/icon.qa.stories.tsx) stories include component sizes, CSS-mask rendering and **Component Legibility**, which compares icons in native controls across densities and backgrounds. **All Icon View Boxes** (`AllIconViewBoxes`) shows each export's frame at its **actual density-based icon size** or a fixed **12px, 16px, 32px or 64px**, with **density selection**, **light/dark** backgrounds and **name filtering**. Both stories inherit the selected density's theme stroke by default. Disable `useDensityStroke` to compare an explicit stroke width. Use All Icon View Boxes to review coverage across the entire catalogue and compare related exports.

Inspect each changed variant and its closest siblings at native **12px and 16px** on **light and dark** backgrounds, plus an enlarged view. Check generated components at the themed defaults **1.5 in high/medium** and **1 in low/touch/mobile**, compare the numeric SVG reference at **0.67**, and inspect the configured weights used by the interface. The story controls support explicit widths up to **1.5**; verify final fitted gaps and bounds at each chosen width.
...
- [ ] Each ordinary export is centered and reaches the 15.5-unit longest painted dimension at width 1.5; the documented brand and full-canvas surface exceptions retain their 16-unit frames.
