...
Generated React icons map these reference widths to the configured primary width **W**, defaulting to **1**, while retaining each stroke's ratio to 0.67. At a rendered size of `P` pixels, the primary stroke occupies `W × P / 16` pixels: 0.75px at 12px with the default width, compared with 0.5025px for the 0.67 reference. The `schedule-time` hands retain their lighter 0.72 proportion. Resizing scales the artwork and its strokes together.
...
The framing transform moves and scales paths, counters and cutouts together while preserving the **0.67-unit primary reference stroke** and every secondary ratio. Generated React components still default to primary width **1**. Thinner strokes occupy slightly less of the frame than the 1.5-width fitting target; inspect acute joins at the actual interface width.
...
The [Icons/Icon/Icon QA](https://github.com/jpmorganchase/salt-ds/blob/main/packages/icons/stories/icon.qa.stories.tsx) stories include component sizes, CSS-mask rendering and **Component Legibility**, which compares icons in native controls across densities and backgrounds with an adjustable stroke width. **All Icon View Boxes** (`AllIconViewBoxes`) shows each export's frame, with controls for **12px, 16px, 32px and 64px**, **light/dark** backgrounds, **stroke width** and **name filtering**. Use it to review coverage across the entire catalogue and compare related exports.

Inspect each changed variant and its closest siblings at native **12px and 16px** on **light and dark** backgrounds, plus an enlarged view. Check generated components at the default width **1**, compare the numeric SVG reference at **0.67**, and inspect the configured weights used by the interface. The Component Legibility control supports tuning up to **1.5**; verify gaps and bounds at each chosen width.
...
- [ ] At the reference, default and selected override widths, fitted paint stays inside the viewBox and final gaps remain useful.
