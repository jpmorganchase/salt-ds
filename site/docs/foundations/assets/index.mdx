...
Each ordinary icon defaults to a centered fit in its 16×16 viewBox, with a **15.5-unit longest painted dimension at stroke width 1.5**. Reviewed optical exceptions can use a smaller span or an offset center. Its aspect ratio is preserved, so narrow icons retain space along their shorter axis. Outline and solid exports are fitted independently. Official brand marks and full-canvas solid checkmark control surfaces retain their 16-unit framing. See [Canvas and optical size](/salt/foundations/assets/icon-design#canvas-and-optical-size) for the design rules.
...
Generated React icons follow the theme's primary stroke width: **1.333333 units (approximately 4/3) in high and medium densities**, and **1 unit in low, touch and mobile densities**. Adjust `--salt-size-icon-strokeWidth` for an application or subtree, or use `--saltIcon-strokeWidth` for an individual icon. When neither variable is defined, the component falls back to 1. Secondary details retain their lighter proportions, and all strokes scale with the icon. See [Icon stroke width](/salt/foundations/size#stroke-width) for the density defaults and examples.

<IconDesignExamples example="stroke" />

At the default 12px icon size in high and medium densities, the 1.333333-unit primary stroke renders at **approximately 1 CSS pixel**. An explicit 1-unit width at the same size renders at 0.75px. The viewBox fit reserves space for widths up to 1.5; thinner strokes occupy slightly less of the frame, especially at sharp joins. Numeric SVG masters retain a 0.67-unit reference stroke, which renders at 0.5025px at 12px. Standalone SVG images and the CSS mask classes in `saltIcons.css` use those baked widths and do not inherit stroke variables from their host element. Filled contours and official brand logos retain their supplied shapes.
