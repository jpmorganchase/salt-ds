...
For detailed information on how to apply duration in design and the use cases, please refer to the [Understanding Duration in User Experience - Insights from the Salt Design System blog post](https://developer.test.aws.jpmchase.net/salt/news/blogs/20250402-duration#). (Internal users only)
...
A progress cursor or [spinner component](https://www.saltdesignsystem.com/salt/components/spinner/examples) can be used to prevent users from wondering whether the action they just took didn’t work or is simply taking time to process.
