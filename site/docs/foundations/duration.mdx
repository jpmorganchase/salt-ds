...
For detailed information on how to apply duration in design and the use cases, please refer to [this blog post](). (Internal users only)
...
A progress cursor or [spinner component](go/salt/components/spinner) can be used to prevent users from wondering whether the action they just took didn’t work or is simply taking time to process.
