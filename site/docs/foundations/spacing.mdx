...
- The amount of spacing placed directly below a heading is dependent on the level of focus placed on the following item.

### Hierarchical spacing

When content directly following a header is a primary focus, for instance the first paragraph of an article, the spacing between the header and the text is relative to the level of the heading. A H1 is followed by `--salt-spacing-300`, H2 by `--salt-spacing-200`, and H3 by `--salt-spacing-100`. For text accompanied by a H4 there is no spacing between the elements. Additionally:
...
- When a heading is followed by a description the spacing is reduced to reinforce the relationship between heading and description.
...
For content following a heading that is not text, such as an image, form, card or data visualisation, the spacing between the elements is treated consistently, as with non-hierarchical spacing. To clarify:
...
- When elements repeat at regular intervals visual rhythm is maintained
- Consistency and rhythm helps people to comprehend and navigate an interface more easily
...
For the purposes of spacing, separators are considered as a type of content and therefore spaced consistently. This means that:

- Separators do not come with any pre-defined spacing of their own.
- `--salt-spacing-300` should appear above and below a separator when placed between sections.
...
Only use separators only if the content cannot be effectively divided in another way. Too many repeating lines can overload a design and create unnecessary visual noise.
