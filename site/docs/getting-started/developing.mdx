...
- Zeroes browser default margin and padding on common block elements (`h1`–`h6`, `p`, `ul`, `ol`, `dl`, `dd`, `blockquote`, `pre`, `figure`, `fieldset`, `hr`).
- Makes headings inherit `font-size` and `font-weight` so bare `<h1>` doesn't break layouts.
- Sets sensible defaults for media (`img`, `svg`, …), form controls, bare `<button>`, `<fieldset>`, `<table>`, and `<a>`.
