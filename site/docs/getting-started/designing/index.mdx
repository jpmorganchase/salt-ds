...
<Callout title="Salt DS JPM Brand Colors, Legacy (UITK) Colors libraries">
The color libraries below provide the color variables for each theme. Using these color variables directly in a design file is not recommended as it does not reflect the way Salt components reference colors in code. Instead please use the semantic variables provided in the Salt DS Theme Figma library.
</Callout>
...
<QuickLinks columns={{ xs: 1, md: 2 }}>
  ...
  >
    To start using the Salt libraries, search for the **@jpmorgan_salt** handle in the Figma Community. Next, publish your library to Figma. Then, proceed with the steps below.
  </QuickLink>
</QuickLinks>
