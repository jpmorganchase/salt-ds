...
Salt offers three themes:

- **JPM Brand** (default)—_recommended for all new projects._
- **Legacy (UITK)**—designed to support the migration of applications from Salt’s predecessor.
- **Chase**—available exclusively to JPMorganChase employees for applications serving Chase customers.

The long-term goal for Legacy users remains transitioning to the JPM Brand theme.
...
### Setting up the JPM Brand theme

Import the base theme CSS plus the JPM Brand layer, then wrap your app in `<SaltProviderNext>`:
...
The JPM Brand theme requires the **Amplitude** web font. JPMC employees can download the font files from [Salt internal resources](https://go/salt-ds-internal/resources/index), then add the following `@font-face` declarations to your application CSS:
