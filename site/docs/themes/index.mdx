...
Salt offers three themes: the default ‘JPM Brand’ for all new projects, ‘Legacy’ for migrating older applications from the previous design system, and ‘Chase’ for cross-brand consumers. The eventual goal for older applications from the previous design system remains transitioning to the JPM Brand theme.
